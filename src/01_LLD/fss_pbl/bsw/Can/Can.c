...
| 2022-03-08 5.11.00  Hnj - CANHAL-839   : Security Event Reporting error detection enhancement
|                     Pl  - CANHAL-798   : Extended RAM Check Implementation for CANbedded
| 2022-05-02 5.11.01  Hnj - ESCAN00111284: Memory corruption during RAM check
|                     Pl  - ESCAN00111527: CanEnableMailboxCommunication API does not enable deactivated CAN MailboxObject
| 2022-06-28 5.11.02  Ped - ESCAN00112044: Wrong CAN Configuration Write Password for TCC70xx derivatives
|                     Ped - CANHAL-1344  : Improved filter handling for SBSW_CAN_LL_128
| 2022-08-12 5.11.03  Pl  - CANHAL-1424  : Reworks for External PIA
| 2023-01-17 5.11.04  Hum - ESCAN00112987: MISRA deviation: MISRA-C:2004 Rule 9.1
|                     Hum - ESCAN00113161: ECC Issue when receiving not suitable RX message to the configured mailbox
|                     Jan - CANCORE-1779 : FETA Analysis
|************************************************************************** */
...
#if (CAN_COREVERSION           != 0x1002u) /* \trace SPEC-1699 */
...
#endif
#if( DRVCAN_MPC5700MCANASR_VERSION != 0x0511)
# error "Source and Header file are inconsistent!"
#endif
...
# if(CAN_GEN_Mpc5700McanAsr_VERSION != 0x0504)     /* CFG5 */
...
# define kCanMCMcanEnableMemAccess            0xFFFFFFFFUL  /*! 0xFFFFFFFF Enable Memory Access to all Register */
# define kCanMCMcan_MCR_CCCE                  0x80000000UL  /*! Clock and RAM change enable */
# define kCanMCMcan_MCR_CI                    0x40000000UL  /*! Change Init */
# define kCanMCMcan_MCR_RINIT                 0x20000000UL  /*! RAM init (not available in A-step) */
# define kCanMCMcan_MCR_RBUSY                 0x10000000UL  /*! RAM BUSY */
# define kCanMCMcanSetMCR_prog_sequence_0     0xC0000000UL  /*! To be able to change the clock settings the following programming sequence needs to be met: */
# define kCanMCMcanSetMCR_prog_sequence_1     0xC00000FFUL  /*! CANn_MCR|=0xC0000000;  change clock settings within MCR; CANn_MCR&=~0xC0000000; */
# define kCanMCMcanSetMCR_prog_sequence_2     0x000000FFUL  /*! after this clock settings for CAN nodes becomes active. */
# define kCanMCMcanDisable_Service_Request    0xFFFFFBFFUL  /*! disable the CAN Interrupt in the SRN */
# define kCanMCMcanSRCR_Mask                  0x00003CFFu   /*! Mask for Service Request Node register */
# define kCanMCMcan_NPCR_DELE                 0x00000400UL /*! enable destructive read on ECR and PSR registers */
...
#define CanLL_TxIsObjFree( hwCh, txObjHandle ) ( ((Cn->TXBC & kCanTXBC_TFQS) > 0UL) ? /* check Fifo/Queue size */ \
            (kCanFalse!=kCanFalse) :  /* N/A: Fifo/Queue exists with size != zero */ \
...
#if defined( C_ENABLE_EXTENDED_STATUS )||defined(CAN_ENABLE_SECURITY_EVENT_REPORTING)
/*
...
# if defined( C_SINGLE_RECEIVE_CHANNEL )
#  define CanLL_HwIsPassive( hwCh ) ((CanRxActualErrorCounter() >= 128u) || (CanTxActualErrorCounter() >= 128u))
# else
#  define CanLL_HwIsPassive( hwCh ) ((CanRxActualErrorCounter(hwCh) >= 128u) || (CanTxActualErrorCounter(hwCh) >= 128u))
# endif
...
# if defined( C_SINGLE_RECEIVE_CHANNEL )
#  define CanLL_HwIsWarning( hwCh ) ((CanRxActualErrorCounter() >= 96u) || (CanTxActualErrorCounter() >= 96u))
# else
#  define CanLL_HwIsWarning( hwCh ) ((CanRxActualErrorCounter(hwCh) >= 96u) || (CanTxActualErrorCounter(hwCh) >= 96u))
# endif
...
#   define CAN_PROTECTED_AREA(ch)        Can_GetChannelBasedProtectedAreaId(ch)
#   define CAN_PROTECTED_AREA_SUB(sub)   Can_GetSubSystemBasedProtectedAreaId(sub)
#  else
    /* Unused Dummy Area for Appl Mode */
#   define CAN_PROTECTED_AREA(ch)        0u
#  endif
#  define CAN_SET_OSVALUE_32_CHANNEL(regAddr, val, mask, localPara, ch) (void)CanLL_WriteProtectedReg32(CAN_CHANNEL_CANPARA_FIRST CAN_PROTECTED_AREA(ch), (regAddr), (val), (mask), (localPara))
# else
#  define CAN_SET_OSVALUE_32_CHANNEL(regAddr, val, mask, localPara, ch) (void)CanLL_WriteReg32(CAN_CHANNEL_CANPARA_FIRST (regAddr), (val), (mask), (localPara))
# endif
...
# if defined(C_ENABLE_MULTIPLE_BASICCAN_TX)
  for (; mailboxHandle < Can_GetTxBasicHandleStop(canHwChannel); mailboxHandle++) /* PRQA S 2994,2996 */ /* MD_Can_ConstValue */ /* FETA_CAN_HL01 */
# endif
  {
# if defined(C_ENABLE_MULTIPLEXED_TRANSMISSION) || defined(C_ENABLE_TX_HW_FIFO)
    for (mailboxElement = 0u; mailboxElement < Can_GetMailboxSize(mailboxHandle); mailboxElement++) /* PRQA S 2880 */ /*  MD_MSR_Unreachable */ /* FETA_CAN_HL01 */
# else
...
# if defined(C_ENABLE_TX_FULLCAN_OBJECTS)
  ...
  for (mailboxHandle = Can_GetTxFullHandleStart(canHwChannel); mailboxHandle < Can_GetTxFullHandleStop(canHwChannel); mailboxHandle++) /* PRQA S 2994,2996 */ /* MD_Can_ConstValue */ /* FETA_CAN_HL01 */
  {
...
# endif
    { /* #27 if normal or Mux Tx search from first to last element 0..1 or 0..3 for free object or one to be cancelled */
      ...
      for (txPara->mailboxElement = 0u; txPara->mailboxElement < mailboxElement_max; txPara->mailboxElement++) /* SBSW_CAN_HL42 */ /* SBSW_CAN_HL42 */ /* FETA_CAN_HL01 */
...
#if ( defined(C_ENABLE_TX_HW_FIFO) && defined(CAN_ENABLE_GENERIC_CONFIRMATION_API2) )
/****************************************************************************
...
#if defined(C_ENABLE_CAN_FD_USED)
    /* calculate frameLen based upon Pdu length as PDU length may be changed in Pretransmit */
    txPara->messageLen = txPara->pdu.length; /* SBSW_CAN_HL42 */
    txPara->frameLen = CAN_DLC2LEN(CAN_LEN2DLC(txPara->messageLen)); /* SBSW_CAN_HL42 */
#endif
...
# endif
  ...
  for (initPara->mailboxHandle = Can_GetTxFullHandleStart(canHwChannel); initPara->mailboxHandle < Can_GetTxFullHandleStop(canHwChannel); initPara->mailboxHandle++) /* PRQA S 2994,2996 */ /* MD_Can_ConstValue */ /* SBSW_CAN_HL36 */ /* SBSW_CAN_HL36 */ /* FETA_CAN_HL01 */
...
V_DEF_FUNC(CAN_LOCAL_INLINE, uint8, STATIC_CODE) CanHL_InitTxBasicCAN( CAN_CHANNEL_CANTYPE_FIRST CanInitParaStructPtr initPara )
{
...
#if defined(C_ENABLE_MULTIPLE_BASICCAN_TX)
  for (; initPara->mailboxHandle < Can_GetTxBasicHandleStop(canHwChannel); initPara->mailboxHandle++) /* PRQA S 2994,2996 */ /* MD_Can_ConstValue */ /* SBSW_CAN_HL36 */ /* FETA_CAN_HL01 */
#endif
  { /* #10 iterate over all logical BasicCAN TX */
    ...
    for (; activeSendObject < activeSendObjectStop; activeSendObject++) /* FETA_CAN_HL03 */
#endif
...
# endif
  ...
  /* ----- Implementation ----------------------------------------------- */
  initOk = kCanOk;
  for (initPara->mailboxHandle = Can_GetRxFullHandleStart(canHwChannel); initPara->mailboxHandle < Can_GetRxFullHandleStop(canHwChannel); initPara->mailboxHandle++) /* PRQA S 2994,2996 */ /* MD_Can_ConstValue */ /* COV_CAN_GENDATA_NOT_IN_ALL_CONFIG */ /* SBSW_CAN_HL36 */ /* SBSW_CAN_HL36 */ /* FETA_CAN_HL01 */
...
# endif
  ...
  /* ----- Implementation ----------------------------------------------- */
  initOk = kCanOk;
  for (initPara->mailboxHandle = Can_GetRxBasicHandleStart(canHwChannel); initPara->mailboxHandle < Can_GetRxBasicHandleStop(canHwChannel); initPara->mailboxHandle++) /* PRQA S 2994,2996 */ /* MD_Can_ConstValue */ /* SBSW_CAN_HL36 */ /* SBSW_CAN_HL36 */ /* FETA_CAN_HL01 */
...
V_DEF_FUNC(CAN_LOCAL_INLINE, void, STATIC_CODE) CanHL_RxBasicCanPolling( CAN_HW_CHANNEL_CANTYPE_ONLY )
{
  ...
  if(CanLL_RxBasicIsGlobalIndPending(CAN_HW_CHANNEL_CANPARA_ONLY) == kCanTrue) /* PRQA S 2991,2995 */ /* MD_Can_ConstValue */ /* COV_CAN_LL_HARDWARE_BEHAVIOUR */
  {
    ...
    for (taskPara.mailboxHandle = Can_GetRxBasicHandleStart(canHwChannel); taskPara.mailboxHandle < Can_GetRxBasicHandleStop(canHwChannel); taskPara.mailboxHandle++) /* PRQA S 2994,2996 */ /* MD_Can_ConstValue */ /* FETA_CAN_HL01 */
...
V_DEF_FUNC(CAN_LOCAL_INLINE, void, STATIC_CODE) CanHL_RxFullCanPolling( CAN_HW_CHANNEL_CANTYPE_ONLY )
{
  ...
  if(CanLL_RxFullIsGlobalIndPending(CAN_HW_CHANNEL_CANPARA_ONLY) == kCanTrue) /* PRQA S 2991,2995 */ /* MD_Can_ConstValue */ /* COV_CAN_LL_HARDWARE_BEHAVIOUR */
  {
    ...
    for (taskPara.mailboxHandle = Can_GetRxFullHandleStart(canHwChannel); taskPara.mailboxHandle < Can_GetRxFullHandleStop(canHwChannel); taskPara.mailboxHandle++ ) /* PRQA S 2994,2996 */ /* MD_Can_ConstValue */ /* FETA_CAN_HL01 */
    {
      taskPara.hwObjHandle = Can_GetMailboxHwHandle(taskPara.mailboxHandle); /* PRQA S 2880 */ /*  MD_MSR_Unreachable */
#  if defined(C_ENABLE_INDIVIDUAL_POLLING) /* COV_CAN_HWOBJINDIVPOLLING */
      /* #30 check individual polling flag */
...
V_DEF_FUNC(CAN_LOCAL_INLINE, void, STATIC_CODE) CanHL_RxQueueExecution( void )
{
  ...
  while ( ( Can_GetRxQueueInfo().Count != (uint16)0 ) && ( loopTimeout < Can_GetSizeOfRxQueueBuffer() ) ) /* FETA_CAN_HL04 */
...
# if defined (C_ENABLE_CANSUBSYSTEMBASEADR) /* COV_CAN_HW_SPECIFIC */
/****************************************************************************
| NAME:             CanLL_InitSubSystemVirtualAddr
****************************************************************************/
/*
|<DataModelStart>| CanLL_InitSubSystemVirtualAddr
Relation_Context:
# CanLL_InitSubSystemVirtualAddr() #
Relation:
OneChOpt
ChannelAmount
VirtualAddressing
 ...
 *
 */
V_DEF_FUNC(CAN_LOCAL_INLINE, void, STATIC_CODE) CanLL_InitSubSystemVirtualAddr(void)
{
  vuint8 subSystemIndex;
  /* #10 SafeBSW check for the amount of subsystems */
#  if( CAN_SAFE_BSW == STD_ON )
  if(Can_GetSizeOfCanSubSystemBaseAdr() != Can_GetSizeOfCanVirtualSubSystemBaseAdr()) /* CM_CAN_LL_115 */ /* PRQA S 3325,2992,2742 */ /* MD_Can_3325_LL,MD_Can_ConstValue,MD_Can_ConstValue */ /* COV_CAN_GENDATA_FAILURE */
  {
    Can_CallDetReportError(CAN_INIT_ID, CAN_E_GENDATA); /* PRQA S 2880 */ /*  MD_MSR_Unreachable */
  }
  else
#  endif
  {
    /* #20 Get virtual base address of subsystems */
    for (subSystemIndex = 0u; subSystemIndex<Can_GetSizeOfCanSubSystemBaseAdr(); subSystemIndex++) /* FETA_CAN_HL01 */
    {
      Can_SetCanVirtualSubSystemBaseAdr(subSystemIndex, (vuint32)(ApplCanPowerOnGetBaseAddress((vuint32)(Can_GetCanSubSystemBaseAdr(subSystemIndex)), sizeof(vuint32)))); /* SBSW_CAN_LL_127 */
    }
  }
}
# endif
/****************************************************************************
| NAME:             CanLL_InitVirtualAddr
****************************************************************************/
/*
|<DataModelStart>| CanLL_InitVirtualAddr
Relation_Context:
# CanLL_InitVirtualAddr() #
Relation:
OneChOpt
ChannelAmount
VirtualAddressing
 ...
 *
 */
V_DEF_FUNC(CAN_LOCAL_INLINE, void, STATIC_CODE) CanLL_InitVirtualAddr(CAN_CHANNEL_CANTYPE_ONLY)
{
  /* #10 Get virtual base address of Mcan controller */
  Can_SetVirtualBasisAdrOfControllerData(canHwChannel, (vuint32)(ApplCanPowerOnGetBaseAddress((vuint32)(Can_GetBasisAdrOfControllerConfig(canHwChannel)), sizeof(vuint32)))) ; /* SBSW_CAN_LL_112 */
  
  /* #20 Get virtual base address of shared message RAM */
  Can_SetStartAdrOfVirtualShmAdr(canHwChannel, (vuint32)(ApplCanPowerOnGetBaseAddress((vuint32)(Can_GetStartAdrOfShmAdr(canHwChannel)), sizeof(vuint32)))) ;    /* SBSW_CAN_LL_103 */
  Can_SetSIDFAOfVirtualShmAdr(canHwChannel, (vuint32)(Can_GetSIDFAOfShmAdr(canHwChannel)) - (vuint32)(Can_GetStartAdrOfShmAdr(canHwChannel)))  ;                /* SBSW_CAN_LL_103 */
  Can_SetSIDFAOfVirtualShmAdr(canHwChannel, (vuint32)(Can_GetSIDFAOfVirtualShmAdr(canHwChannel)) + (vuint32)(Can_GetStartAdrOfVirtualShmAdr(canHwChannel)))  ;  /* SBSW_CAN_LL_103 */
  Can_SetXIDFAOfVirtualShmAdr(canHwChannel, (vuint32)(Can_GetXIDFAOfShmAdr(canHwChannel)) - (vuint32)(Can_GetStartAdrOfShmAdr(canHwChannel)))  ;                /* SBSW_CAN_LL_103 */
  Can_SetXIDFAOfVirtualShmAdr(canHwChannel, (vuint32)(Can_GetXIDFAOfVirtualShmAdr(canHwChannel)) + (vuint32)(Can_GetStartAdrOfVirtualShmAdr(canHwChannel)))  ;  /* SBSW_CAN_LL_103 */
  Can_SetRXF0AOfVirtualShmAdr(canHwChannel, (vuint32)(Can_GetRXF0AOfShmAdr(canHwChannel)) - (vuint32)(Can_GetStartAdrOfShmAdr(canHwChannel)))  ;                /* SBSW_CAN_LL_103 */
  Can_SetRXF0AOfVirtualShmAdr(canHwChannel, (vuint32)(Can_GetRXF0AOfVirtualShmAdr(canHwChannel)) + (vuint32)(Can_GetStartAdrOfVirtualShmAdr(canHwChannel)))  ;  /* SBSW_CAN_LL_103 */
  Can_SetRXF1AOfVirtualShmAdr(canHwChannel, (vuint32)(Can_GetRXF1AOfShmAdr(canHwChannel)) - (vuint32)(Can_GetStartAdrOfShmAdr(canHwChannel)))  ;                /* SBSW_CAN_LL_103 */
  Can_SetRXF1AOfVirtualShmAdr(canHwChannel, (vuint32)(Can_GetRXF1AOfVirtualShmAdr(canHwChannel)) + (vuint32)(Can_GetStartAdrOfVirtualShmAdr(canHwChannel)))  ;  /* SBSW_CAN_LL_103 */
  Can_SetRXBAOfVirtualShmAdr(canHwChannel, (vuint32)(Can_GetRXBAOfShmAdr(canHwChannel)) - (vuint32)(Can_GetStartAdrOfShmAdr(canHwChannel)))  ;                  /* SBSW_CAN_LL_103 */
  Can_SetRXBAOfVirtualShmAdr(canHwChannel, (vuint32)(Can_GetRXBAOfVirtualShmAdr(canHwChannel)) + (vuint32)(Can_GetStartAdrOfVirtualShmAdr(canHwChannel)))  ;    /* SBSW_CAN_LL_103 */
  Can_SetTXBAOfVirtualShmAdr(canHwChannel, (vuint32)(Can_GetTXBAOfShmAdr(canHwChannel)) - (vuint32)(Can_GetStartAdrOfShmAdr(canHwChannel)))  ;                  /* SBSW_CAN_LL_103 */
  Can_SetTXBAOfVirtualShmAdr(canHwChannel, (vuint32)(Can_GetTXBAOfVirtualShmAdr(canHwChannel)) + (vuint32)(Can_GetStartAdrOfVirtualShmAdr(canHwChannel)))  ;    /* SBSW_CAN_LL_103 */
  Can_SetTXEFAOfVirtualShmAdr(canHwChannel, (vuint32)(Can_GetTXEFAOfShmAdr(canHwChannel)) - (vuint32)(Can_GetStartAdrOfShmAdr(canHwChannel)))  ;                /* SBSW_CAN_LL_103 */
  Can_SetTXEFAOfVirtualShmAdr(canHwChannel, (vuint32)(Can_GetTXEFAOfVirtualShmAdr(canHwChannel)) + (vuint32)(Can_GetStartAdrOfVirtualShmAdr(canHwChannel)))  ;  /* SBSW_CAN_LL_103 */
 ...
 *
 */
V_DEF_FUNC(CAN_LOCAL_INLINE, vuint8, STATIC_CODE) CanLL_WriteReg32( CAN_CHANNEL_CANTYPE_FIRST CanChipMsgPtr32 regPtr, vuint32 value, vuint32 readMask, CanInitParaStructPtr initPara ) /* PRQA S 3673 */ /* MD_Can_NoneConstParameterPointer */
{
 ...
 *
 */
V_DEF_FUNC(CAN_LOCAL_INLINE, vuint8, STATIC_CODE) CanLL_WriteProtectedReg32(CAN_CHANNEL_CANTYPE_FIRST vuint16 area, CanChipMsgPtr32 regPtr, vuint32 value, vuint32 readMask, CanInitParaStructPtr initPara ) /* PRQA S 3673, 3219 */ /* MD_Can_NoneConstParameterPointer, MD_Can_HL_UnusedFunction */ /* COV_CAN_PROTECTED_WRITE_LL */
{
  vuint8 result = kCanFalse;
  /* #10 write register value */
...
# endif
  ...
  if ((Cn->CCCR & kCanCCCR_INIT) != kCanCCCR_INIT ) /* If Init-Bit is not set */ /* PRQA S 2842,2843 */ /* MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */
  {
    ...
    CanLL_ApplCanTimerStart(kCanLoopInit); /* PRQA S 2842 */ /* MD_Can_Assertion */ /* SBSW_CAN_LL_101 */
    while( ((Cn->CCCR & kCanCCCR_INIT) == 0UL) && (CanLL_ApplCanTimerLoop(kCanLoopInit) !=  CAN_NOT_OK) ) /* PRQA S 1881,4558,3415,0303,2842,2843 */ /* MD_MSR_AutosarBoolean,MD_Can_4558_RHsideEffect,MD_Can_4558_RHsideEffect,MD_Can_HwAccess,MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */ /* COV_CAN_HW_SPECIFIC */ /* FETA_CAN_LL01 */
    {/* Assure that INIT has been accepted. */}
    CanLL_ApplCanTimerEnd(kCanLoopInit); /* SBSW_CAN_LL_101 */ /* PRQA S 2842 */ /* MD_Can_Assertion */
  }

  /* Configuration Change Enable */
  Cn->CCCR |= kCanCCCR_CCE; /* SBSW_CAN_LL_100 */ /* PRQA S 2842,2843 */ /* MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */
  CanLL_ApplCanTimerStart(kCanLoopInit); /* PRQA S 2842 */ /* MD_Can_Assertion */ /* SBSW_CAN_LL_101 */
  while( ((Cn->CCCR & kCanCCCR_CCE) == 0UL) && (CanLL_ApplCanTimerLoop(kCanLoopInit) !=  CAN_NOT_OK) ) /* PRQA S 1881,4558,3415,0303,2842,2843 */ /* MD_MSR_AutosarBoolean,MD_Can_4558_RHsideEffect,MD_Can_4558_RHsideEffect,MD_Can_HwAccess,MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */ /* COV_CAN_HW_SPECIFIC */ /* FETA_CAN_LL01 */
  {/* Assure that CCE has been accepted. */}
  CanLL_ApplCanTimerEnd(kCanLoopInit); /* SBSW_CAN_LL_101 */ /* PRQA S 2842 */ /* MD_Can_Assertion */
  
  Cn->IE  = 0x00000000u; /* PRQA S 2842,2843 */ /* MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */ /* SBSW_CAN_LL_100 */  /* Disable MCAN Interrupts */
  ...
  for(indx = 0x00u; indx < 4u; indx++)
    { 
      ...
      Cn->CCCR = (((kCanRegCheckMaskCCCR ^ kCanRegCheckMaskCCCR_RP) & CanRegCheckValues32bit[indx])|kCanRegCheckMaskCCCR_RP); /* SBSW_CAN_LL_100 */ /* PRQA S 2842,2843 */ /* MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */
       
      /* Data Bit Timing & Prescaler Register (FBTP),RP */ 
      Cn->FBTP = (kCanRegCheckMaskFBTP & CanRegCheckValues32bit[indx]); /* SBSW_CAN_LL_100 */ /* PRQA S 2842,2843 */ /* MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */

      /* RAM Watchdog (RWD), RP */
      Cn->RWD  = (kCanRegCheckMaskRWD  & CanRegCheckValues32bit[indx]); /* SBSW_CAN_LL_100 */ /* PRQA S 2842,2843 */ /* MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */
      /* Bit Timing and Prescaler Register (BTP/NBTP), RP */
      Cn->BTP = (kCanRegCheckMaskBTP  & CanRegCheckValues32bit[indx]); /* SBSW_CAN_LL_100 */ /* PRQA S 2842,2843 */ /* MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */
      
      /* Timestamp Counter Configuration (TSCC), RP */
      Cn->TSCC = (kCanRegCheckMaskTSCC  & CanRegCheckValues32bit[indx]); /* SBSW_CAN_LL_100 */  /*! Not supported but initialized */ /* PRQA S 2842,2843 */ /* MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */
      /* Timeout Counter Configuration (TOCC), RP */
      Cn->TOCC = (kCanRegCheckMaskTOCC  & CanRegCheckValues32bit[indx]); /* SBSW_CAN_LL_100 */  /*! Not supported but initialized */ /* PRQA S 2842,2843 */ /* MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */
      /* Timeout Counter Value (TOCV), RC */ 
      
      /* Transmitter Delay Compensation Register (TDCR), RP */
      {
#  if ( CAN_MCAN_REVISION >= 0x0310UL ) /* COV_CAN_HW_SPECIFIC */ 
        Cn->TDCR = (kCanRegCheckMaskTDCR  & CanRegCheckValues32bit[indx]); /* SBSW_CAN_LL_100 */ /* PRQA S 2842,2843 */ /* MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */
#  endif /* MCAN Core Release < 0x0310UL */
      }
    
      /* Global Filter Configuration (GFC),RP */
      Cn->GFC = (kCanRegCheckMaskGFC  & CanRegCheckValues32bit[indx]); /* SBSW_CAN_LL_100 */ /* PRQA S 2842,2843 */ /* MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */
      /* Standard ID Filter Configuration (SIDFC), RP */
      Cn->SIDFC = (kCanRegCheckMaskSIDFC  & CanRegCheckValues32bit[indx]); /* SBSW_CAN_LL_100 */ /* PRQA S 2842,2843 */ /* MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */
      /* Extended ID Filter Configuration (XIDFC), RP */
      Cn->XIDFC = (kCanRegCheckMaskXIDFC  & CanRegCheckValues32bit[indx]); /* SBSW_CAN_LL_100 */ /* PRQA S 2842,2843 */ /* MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */
      /* Extended ID AND Mask (XIDAM), RP */
      Cn->XIDAM = (kCanRegCheckMaskXIDAM  & CanRegCheckValues32bit[indx]); /* SBSW_CAN_LL_100 */ /* PRQA S 2842,2843 */ /* MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */
      /* Rx FIFO 0 Configuration (RXF0C), RP */
      Cn->RXF0C.R = (kCanRegCheckMaskRXF0C  & CanRegCheckValues32bit[indx]); /* SBSW_CAN_LL_100 */ /* PRQA S 2842,2843 */ /* MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */
      /* Rx FIFO 0 Acknowledge (RXF0A), RW */
      Cn->RXF0A = (kCanRegCheckMaskRXF0A  & CanRegCheckValues32bit[indx]); /* SBSW_CAN_LL_100 */ /* PRQA S 2842,2843 */ /* MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */
      /* Rx Buffer Configuration (RXBC), RP */ 
      Cn->RXBC = (kCanRegCheckMaskRXBC  & CanRegCheckValues32bit[indx]); /* SBSW_CAN_LL_100 */ /* PRQA S 2842,2843 */ /* MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */
      /* Rx FIFO 1 Configuration (RXF1C), RP */
      Cn->RXF1C.R = (kCanRegCheckMaskRXF1C  & CanRegCheckValues32bit[indx]); /* SBSW_CAN_LL_100 */ /* PRQA S 2842,2843 */ /* MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */
      /* Rx FIFO 1 Acknowledge (RXF1A), RW */
      Cn->RXF1A = (kCanRegCheckMaskRXF1A  & CanRegCheckValues32bit[indx]); /* SBSW_CAN_LL_100 */ /* PRQA S 2842,2843 */ /* MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */
      /* Rx Buffer / FIFO Element Size Configuration (RXESC), RP */ 
      Cn->RXESC.R = (kCanRegCheckMaskRXESC  & CanRegCheckValues32bit[indx]); /* SBSW_CAN_LL_100 */ /* PRQA S 2842,2843 */ /* MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */
      /* Tx Buffer Configuration (TXBC) */
      Cn->TXBC = (kCanRegCheckMaskTXBC  & CanRegCheckValues32bit[indx]); /* SBSW_CAN_LL_100 */ /* PRQA S 2842,2843 */ /* MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */
      /* Tx Buffer Element Size Configuration (TXESC) */ 
      Cn->TXESC = (kCanRegCheckMaskTXESC  & CanRegCheckValues32bit[indx]); /* SBSW_CAN_LL_100 */ /* PRQA S 2842,2843 */ /* MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */
      /* Tx Buffer Transmission Interrupt Enable (TXBTIE), RW */
      Cn->TXBTIE = (kCanRegCheckMaskTXBTIE  & CanRegCheckValues32bit[indx]); /* SBSW_CAN_LL_100 */  /* PRQA S 2986,4558,2842,2843 */ /* MD_Can_4558_RHsideEffect,MD_Can_4558_RHsideEffect,MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */
      /* Tx Event FIFO Configuration (TXEFC) */
      Cn->TXEFC = (kCanRegCheckMaskTXEFC  & CanRegCheckValues32bit[indx]); /* SBSW_CAN_LL_100 */  /* Not supported but initialized */ /* PRQA S 2842,2843 */ /* MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */
    
      /* #30 Check if register values are valid  */ 
      if(
          ( (Cn->FBTP    & kCanRegCheckMaskFBTP)  != (kCanRegCheckMaskFBTP  & CanRegCheckValues32bit[indx]))   || /* PRQA S 2842,2843 */ /* MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */
          ( (Cn->CCCR & (kCanRegCheckMaskCCCR ^ kCanRegCheckMaskCCCR_RP))!= ((kCanRegCheckMaskCCCR ^ kCanRegCheckMaskCCCR_RP) & CanRegCheckValues32bit[indx]) ) || /* PRQA S 2842,2843 */ /* MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */
          ( (Cn->BTP     & kCanRegCheckMaskBTP)   != (kCanRegCheckMaskBTP   & CanRegCheckValues32bit[indx]))   || /* PRQA S 2842,2843 */ /* MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */
          ...
          ( (Cn->RWD     & kCanRegCheckMaskRWD)   != (kCanRegCheckMaskRWD   & CanRegCheckValues32bit[indx]))   || /* PRQA S 2842,2843 */ /* MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */
          ( (Cn->TSCC    & kCanRegCheckMaskTSCC)  != (kCanRegCheckMaskTSCC  & CanRegCheckValues32bit[indx]))   || /* PRQA S 2842,2843 */ /* MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */
          ( (Cn->TOCC    & kCanRegCheckMaskTOCC)  != (kCanRegCheckMaskTOCC  & CanRegCheckValues32bit[indx]))   || /* PRQA S 2842,2843 */ /* MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */
# if ( CAN_MCAN_REVISION >= 0x0310UL ) /* COV_CAN_HW_SPECIFIC */
          ( (Cn->TDCR    & kCanRegCheckMaskTDCR)  != (kCanRegCheckMaskTDCR  & CanRegCheckValues32bit[indx]))   || /* PRQA S 2842,2843 */ /* MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */
# endif /* MCAN Core Release < 0x0310UL */ 
          ( (Cn->GFC     & kCanRegCheckMaskGFC)   != (kCanRegCheckMaskGFC   & CanRegCheckValues32bit[indx]))   || /* PRQA S 2842,2843 */ /* MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */
          ( (Cn->SIDFC   & kCanRegCheckMaskSIDFC) != (kCanRegCheckMaskSIDFC & CanRegCheckValues32bit[indx]))   || /* PRQA S 2842,2843 */ /* MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */
          ( (Cn->XIDFC   & kCanRegCheckMaskXIDFC) != (kCanRegCheckMaskXIDFC & CanRegCheckValues32bit[indx]))   || /* PRQA S 2842,2843 */ /* MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */
          ( (Cn->XIDAM   & kCanRegCheckMaskXIDAM) != (kCanRegCheckMaskXIDAM & CanRegCheckValues32bit[indx]))   || /* PRQA S 2842,2843 */ /* MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */
          ( (Cn->RXF0C.R & kCanRegCheckMaskRXF0C) != (kCanRegCheckMaskRXF0C & CanRegCheckValues32bit[indx]))   || /* PRQA S 2842,2843 */ /* MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */
          ( (Cn->RXF0A   & kCanRegCheckMaskRXF0A) != (kCanRegCheckMaskRXF0A & CanRegCheckValues32bit[indx]))   || /* PRQA S 2842,2843 */ /* MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */
          ( (Cn->RXBC    & kCanRegCheckMaskRXBC)  != (kCanRegCheckMaskRXBC  & CanRegCheckValues32bit[indx]))   || /* PRQA S 2842,2843 */ /* MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */
          ( (Cn->RXF1C.R & kCanRegCheckMaskRXF1C) != (kCanRegCheckMaskRXF1C & CanRegCheckValues32bit[indx]))   || /* PRQA S 2842,2843 */ /* MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */
          ( (Cn->RXF1A   & kCanRegCheckMaskRXF1A) != (kCanRegCheckMaskRXF1A & CanRegCheckValues32bit[indx]))   || /* PRQA S 2842,2843 */ /* MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */
          ( (Cn->RXESC.R & kCanRegCheckMaskRXESC) != (kCanRegCheckMaskRXESC & CanRegCheckValues32bit[indx]))   || /* PRQA S 2842,2843 */ /* MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */
          ( (Cn->TXBC    & kCanRegCheckMaskTXBC)  != (kCanRegCheckMaskTXBC  & CanRegCheckValues32bit[indx]))   || /* PRQA S 2842,2843 */ /* MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */
          ( (Cn->TXESC   & kCanRegCheckMaskTXESC) != (kCanRegCheckMaskTXESC & CanRegCheckValues32bit[indx]))   || /* PRQA S 2842,2843 */ /* MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */
          ( (Cn->TXBTIE  & kCanRegCheckMaskTXBTIE)!= (kCanRegCheckMaskTXBTIE& CanRegCheckValues32bit[indx]))   ||  /* PRQA S 2986,4558,2842,2843 */ /* MD_Can_4558_RHsideEffect,MD_Can_4558_RHsideEffect,MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */
          ( (Cn->TXEFC   & kCanRegCheckMaskTXEFC) != (kCanRegCheckMaskTXEFC & CanRegCheckValues32bit[indx]))    ) /* COV_CAN_HARDWARE_FAILURE */ /* PRQA S 2842,2843 */ /* MD_Can_Assertion,MD_Can_HL_DereferencePointerValue */
          {
            ret_val = kCanTrue; 
          }
...
V_DEF_FUNC(CAN_LOCAL_INLINE, vuint8, STATIC_CODE) CanLL_InitBeginSetRegisters( CAN_CHANNEL_AND_HW_CHANNEL_CANTYPE_FIRST CanInitParaStructPtr initPara )
{
  ...
  result = CanLL_SetAllRegister(CAN_CHANNEL_CANPARA_FIRST initPara); /* SBSW_CAN_LL_110 */ /* fill in all CAN registers */

  CanLL_SetRegisterMcMcan(CAN_CHANNEL_CANPARA_FIRST initPara); /* SBSW_CAN_LL_110 */
  /* #20 set MCAN filter tables */
  result &= CanLL_SetAllFilter(CAN_CHANNEL_CANPARA_FIRST initPara); /* SBSW_CAN_LL_110 */ /* fill in CAN Message RAM */
...
{
  CanObjectHandle mailboxElement = 0; /* PRQA S 2981 */ /* MD_Can_RedundantInit */
#if defined(C_ENABLE_MULTIPLEXED_TRANSMISSION) || defined(C_ENABLE_TX_HW_FIFO)
  for(; mailboxElement < Can_GetMailboxSize(initPara->mailboxHandle); mailboxElement++) /* FETA_CAN_HL01 */
#endif
  {
    /* #10 get the Message RAM location */
    initPara->pTXB =  (CanElmType*) &ShmTXB(initPara->hwObjHandle + mailboxElement); /* PRQA S 0303,0310,312,1891,4391,2985 */ /* MD_Can_HwAccess,MD_Can_PointerCast,MD_Can_NoneVolatilePointerCast,MD_Can_1891_LL,MD_Can_IntegerCast,MD_Can_ConstValue */ /* SBSW_CAN_LL_109 */
#if( CAN_SAFE_BSW == STD_ON ) 
    if( ((vuint32)initPara->pTXB < (vuint32)Can_GetStartAdrOfShmAdr(canHwChannel)) || ((vuint32)&ShmTXB(initPara->hwObjHandle + mailboxElement +1u) >= (vuint32)Can_GetStopAdrOfShmAdr(canHwChannel) ) ) /* PRQA S 0303,4391,2985 */ /* MD_Can_HwAccess,MD_Can_IntegerCast,MD_Can_ConstValue */ /* CM_CAN_LL_103 */ /* COV_CAN_GENDATA_FAILURE */
    { /* SilentBSW check */
...
#endif
    {
      ...
      initPara->txbtie |= (vuint32)(kCanACTIVE << (initPara->hwObjHandle + mailboxElement)); /* PRQA S 2985 */ /* MD_Can_ConstValue */ /* allow CAN Tx Buffer confirmation and cancelation INTs */ /* SBSW_CAN_LL_109 */ 
#endif
    }
  } /* end for loop */
  return(kCanOk);
...
*/
/* CODE CATEGORY 4 START */
V_DEF_FUNC(CAN_LOCAL_INLINE, void, STATIC_CODE) CanLL_InitMailboxTxDeactivate( CAN_CHANNEL_AND_HW_CHANNEL_CANTYPE_FIRST CanInitParaStructPtr initPara ) /* PRQA S 3673 */ /* MD_Can_NoneConstParameterPointer */ /* COV_CAN_GENDATA_NOT_IN_ALL_CONFIG */
{
...
#  if defined( C_ENABLE_STD_ID ) && (kCanNumberOfStandardFilter > 0)  
  for(index = 0u; index < Can_GetCanSIDFC_LSSOfCanSIDFC(canHwChannel); index++)  /* SBSW_CAN_LL_109 */ /* COV_CAN_GENDATA_NOT_IN_ALL_CONFIG */ /* FETA_CAN_HL01 */
  {
...
#   if( CAN_SAFE_BSW == STD_ON )
      if( ((vuint32)&ShmSIDF(index).S0 <  (vuint32)Can_GetStartAdrOfShmAdr(canHwChannel)) || /* PRQA S 0303 */ /* MD_Can_HwAccess */ /* CM_CAN_LL_103 */ /* COV_CAN_GENDATA_FAILURE */
          ((vuint32)&ShmSIDF(index).S0 >= (vuint32)Can_GetStopAdrOfShmAdr(canHwChannel) ) )  /* PRQA S 0303 */ /* MD_Can_HwAccess */ /* CM_CAN_LL_103 */ /* COV_CAN_GENDATA_FAILURE */
      { /* SilentBSW check */
        Can_CallDetReportError(CAN_INIT_ID, CAN_E_GENDATA);
      }
      else
...
#  if defined(C_ENABLE_EXTENDED_ID) && (kCanNumberOfExtendedFilter > 0)
  for(index = 0u; index < Can_GetCanXIDFC_LSEOfCanXIDFC(canHwChannel); index++)  /* SBSW_CAN_LL_109 */ /* COV_CAN_GENDATA_NOT_IN_ALL_CONFIG */ /* FETA_CAN_HL01 */
  { 
    ...
    {/* Disable the Filter element */
#   if( CAN_SAFE_BSW == STD_ON )
      if( ((vuint32)&ShmXIDF(index).F0 <  (vuint32)Can_GetStartAdrOfShmAdr(canHwChannel)) || /* PRQA S 0303 */ /* MD_Can_HwAccess */ /* CM_CAN_LL_103 */ /* COV_CAN_GENDATA_FAILURE */
          ((vuint32)&ShmXIDF(index).F0 >= (vuint32)Can_GetStopAdrOfShmAdr(canHwChannel) ) )  /* PRQA S 0303 */ /* MD_Can_HwAccess */ /* CM_CAN_LL_103 */ /* COV_CAN_GENDATA_FAILURE */
      { /* SilentBSW check */
        Can_CallDetReportError(CAN_INIT_ID, CAN_E_GENDATA);
      }
      else
...
#  if defined( C_ENABLE_STD_ID ) && (kCanNumberOfStandardFilter > 0)  /* at least one std filter is available */
  ...
  for(index = 0u; index < Can_GetCanSIDFC_LSSOfCanSIDFC(canHwChannel); index++) /* COV_CAN_GENDATA_NOT_IN_ALL_CONFIG */ /* FETA_CAN_HL01 */
  { 
    /* If Filter is configured to ( RXFIFO0 OR RXFIFO1) AND The filter offset matches the HwObjHandle */
    if ( ( (((vuint32) (ShmSIDF(index)).S0 & kCanSFE_SFEC) == kCanSFE_SFEC_F0 ) && ( initPara->hwObjHandle == 0u)) || /* PRQA S 0303 */ /* MD_Can_HwAccess */
         ( (((vuint32) (ShmSIDF(index)).S0 & kCanSFE_SFEC) == kCanSFE_SFEC_F1 ) && ( initPara->hwObjHandle == 1u))   /* PRQA S 0303 */ /* MD_Can_HwAccess */
...
#  if defined(C_ENABLE_EXTENDED_ID) && (kCanNumberOfExtendedFilter > 0)
  ...
  for(index = 0u; index < Can_GetCanXIDFC_LSEOfCanXIDFC(canHwChannel); index++) /* COV_CAN_GENDATA_NOT_IN_ALL_CONFIG */ /* FETA_CAN_HL01 */
  { 
    /* If Filter is configured to ( RXFIFO0 OR RXFIFO1) AND The filter offset matches the HwObjHandle */
    if ( ( (((vuint32) (ShmXIDF(index)).F0 & kCanEFE_EFEC) == kCanEFE_EFEC_F0 ) && ( initPara->hwObjHandle == 0u)) || /* PRQA S 0303 */ /* MD_Can_HwAccess */
         ( (((vuint32) (ShmXIDF(index)).F0 & kCanEFE_EFEC) == kCanEFE_EFEC_F1 ) && ( initPara->hwObjHandle == 1u))  /* PRQA S 0303 */ /* MD_Can_HwAccess */
       ) /* PRQA S 0303 */ /* MD_Can_HwAccess */ /* COV_CAN_DISABLE_RX_MAILBOX */
    { /* Disable the filter element */
#    if( CAN_SAFE_BSW == STD_ON )
      if( ((vuint32)&ShmXIDF(index).F0 <  (vuint32)Can_GetStartAdrOfShmAdr(canHwChannel)) || /* PRQA S 0303 */ /* MD_Can_HwAccess */ /* CM_CAN_LL_103 */ /* COV_CAN_GENDATA_FAILURE */
...
# if( kCanNumberOfStandardFilter > 0 ) 
  ...
  (void)CanLL_WriteReg32(CAN_CHANNEL_CANPARA_FIRST &(Cn->SIDFC), (vuint32)( ((vuint32)(initPara->stdFeCount) << 16) | ((vuint32)Can_GetCanSIDFC_FLSSAOfCanSIDFC(canHwChannel)) ), kCanRegCheckMaskSIDFC, initPara);     /* SBSW_CAN_LL_100 */     /* PRQA S 0303 */ /* MD_Can_HwAccess */
# else
  /* No BasicCAN filter available */
#  if( kCanNumberOfRxObjects > 0 ) /* COV_CAN_COMPATIBILITY */
  /* FullCAN filter available */
  (void)CanLL_WriteReg32(CAN_CHANNEL_CANPARA_FIRST &(Cn->SIDFC), (vuint32)( ((vuint32)(initPara->stdFeCount) << 16) | (((vuint32)(Can_GetCanShmAdr_SIDFAOfCanShmAdr(canHwChannel)) & 0x0000FFFFUL)) ), kCanRegCheckMaskSIDFC, initPara);     /* SBSW_CAN_LL_100 */     /* PRQA S 0303 */ /* MD_Can_HwAccess */
#  else
  (void)CanLL_WriteReg32(CAN_CHANNEL_CANPARA_FIRST &(Cn->SIDFC), (vuint32)(0x00000000UL), kCanRegCheckMaskSIDFC, initPara);    /* SBSW_CAN_LL_100 */    /* PRQA S 0303 */ /* MD_Can_HwAccess */  /* LSS zero means no standard filters at all available */
#  endif
# endif
#endif
...
# if (kCanNumberOfExtendedFilter > 0) /* COV_CAN_COMPATIBILITY */
  /* BasicCAN filter available */
  (void)CanLL_WriteReg32(CAN_CHANNEL_CANPARA_FIRST &(Cn->XIDFC), (vuint32)( ((vuint32)(initPara->extFeCount) << 16) | ((vuint32)(Can_GetCanXIDFC_FLESAOfCanXIDFC(canHwChannel))) ), kCanRegCheckMaskXIDFC, initPara);     /* SBSW_CAN_LL_100 */     /* PRQA S 0303 */ /* MD_Can_HwAccess */
# else
  /* No BasicCAN filter available */
#  if( kCanNumberOfRxObjects > 0 ) /* COV_CAN_COMPATIBILITY */
  /* FullCAN filter available */
...
# endif
  ...
  (void)CanLL_WriteReg32(CAN_CHANNEL_CANPARA_FIRST &(Cn->TXBTIE), (vuint32)(initPara->txbtie), kCanRegCheckMaskTXBTIE, initPara); /* allow CAN Tx Buffer confirmation INTs */ /* PRQA S 0303 */ /* MD_Can_HwAccess */ /* SBSW_CAN_LL_100 */ 
  (void)CanLL_WriteReg32(CAN_CHANNEL_CANPARA_FIRST &(Cn->TXBCIE), (vuint32)(initPara->txbtie), kCan_ALL_ONE, initPara); /* allow CAN Tx Buffer cancellation INTs */ /* PRQA S 0303 */ /* MD_Can_HwAccess */ /* SBSW_CAN_LL_100 */ 

  Can_GetCanIntEnable(channel) = initPara->ie;  /* SBSW_CAN_LL_112 */  /* keep for usage within ISR and SetControllerMode */

  /* If not PowerOn Init */
  if ( (Can_GetLogStatus(canHwChannel) & kCanHwIsInit) == kCanHwIsInit ){
    (void)CanLL_WriteReg32(CAN_CHANNEL_CANPARA_FIRST &(Cn->IE), (vuint32)(initPara->canInterruptOldStatus), kCanRegCheckMaskIE, initPara); /* PRQA S 0303 */ /* MD_Can_HwAccess */ /* SBSW_CAN_LL_100 */ 
  }
  else
  {
    /* Power On */
    ...
    {
      (void)CanLL_WriteReg32(CAN_CHANNEL_CANPARA_FIRST &(Cn->IE), (vuint32)(initPara->ie), kCanRegCheckMaskIE, initPara); /* PRQA S 0303 */ /* MD_Can_HwAccess */ /* SBSW_CAN_LL_100 */ 
    }
    else
    {
      (void)CanLL_WriteReg32(CAN_CHANNEL_CANPARA_FIRST &(Cn->IE), (vuint32)kCanIE_DISABLE_ALL, kCanRegCheckMaskIE, initPara); /* PRQA S 0303 */ /* MD_Can_HwAccess */ /* SBSW_CAN_LL_100 */ 
    }
  ...
  /* The Protocol Exception Event is signalled via PSR.ACT (and PSR.PXE if MCAN Rev. >=3.1.0 ).
    ...
    {
      (void)CanLL_WriteReg32(CAN_CHANNEL_CANPARA_FIRST &(Cn->IE), (vuint32)(Cn->IE | (kCanIR_TOO)), kCanRegCheckMaskIE, initPara); /* Enable timeout interrupt */ /* PRQA S 0303 */ /* MD_Can_HwAccess */ /* SBSW_CAN_LL_100 */ 
    }
#endif
  }
...
{
  vuint8   ret_val;
  CanObjectHandle mailboxElement = 0;
  volatile vuint32* p2Shm = V_NULL; /* PRQA S 2981 */ /* MD_Can_RedundantInit */

  ret_val = kCanFalse; /* Assume That The Mailbox is not corrupt */

# if defined(C_ENABLE_MULTIPLEXED_TRANSMISSION) || defined(C_ENABLE_RX_BASICCAN_OBJECTS) || defined(C_ENABLE_TX_HW_FIFO)
  for (;(( mailboxElement < Can_GetMailboxSize(initPara->mailboxHandle) ) && (ret_val == kCanFalse)); mailboxElement++) /* COV_CAN_HARDWARE_FAILURE */ /* FETA_CAN_HL01 */
# endif
  {

    /* If TX Buffer */
    if((((initPara->mailboxHandle) < CAN_HL_MB_TX_BASIC_STOPINDEX(canHwChannel)) && ((initPara->mailboxHandle) >= CAN_HL_MB_TX_BASIC_STARTINDEX(canHwChannel))) || /* PRQA S 2992,2996 */ /* MD_Can_ConstValue */
       (((initPara->mailboxHandle) < CAN_HL_MB_TX_FULL_STOPINDEX(canHwChannel)) && ((initPara->mailboxHandle) >= CAN_HL_MB_TX_FULL_STARTINDEX(canHwChannel))))/* PRQA S 2992,2996 */ /* MD_Can_ConstValue */ /* COV_CAN_HARDWARE_FAILURE */
    {
...
# if defined( C_ENABLE_RX_FULLCAN_OBJECTS )
    else if (((initPara->mailboxHandle) < CAN_HL_MB_RX_FULL_STOPINDEX(canHwChannel)) && ((initPara->mailboxHandle) >= CAN_HL_MB_RX_FULL_STARTINDEX(canHwChannel))) /* PRQA S 2004 */ /* MD_Can_NoElseAfterIf */ /* COV_CAN_HARDWARE_FAILURE */
    {
...
# if defined(C_ENABLE_RX_BASICCAN_OBJECTS)
    ...
    {
      /* Rx FIFO 0 */
      if(initPara->mailboxHandle == CAN_HL_MB_RX_BASIC_STARTINDEX(canHwChannel)) /* COV_CAN_GENDATA_NOT_IN_ALL_CONFIG */
      {
...
#  if defined( C_ENABLE_MULTIPLE_BASICCAN )
      else if(initPara->mailboxHandle == (CAN_HL_MB_RX_BASIC_STARTINDEX(canHwChannel) + 1u) ) /* COV_CAN_GENDATA_NOT_IN_ALL_CONFIG */
      {
...
# endif
    ...
    { /* found valid handle */
      if( ((vuint32)p2Shm < (vuint32)Can_GetStartAdrOfShmAdr(canHwChannel)) || ((vuint32)p2Shm >= (vuint32)Can_GetStopAdrOfShmAdr(canHwChannel) ) ) /* PRQA S 0306,0303 */ /* MD_Can_0306_HWaccess_LL,MD_Can_HwAccess */ /* CM_CAN_LL_103 */ /* COV_CAN_GENDATA_FAILURE */
      { 
      ...
      else
      {
        ...
        for(i = 0u; (i < 3u) && (ret_val == kCanFalse); i++) /* COV_CAN_HARDWARE_FAILURE */ /* FETA_CAN_LL02 */
        { /* PRQA S 0303 QAC_Can_0303 */ /* MD_Can_HwAccess */
          vuint8 j;
# if defined (C_ENABLE_CAN_FD_FULL)
          for(j=0u ; (j < ((Can_GetMaxDataLenOfMailbox(initPara->mailboxHandle)/4u) + (8u/4u))) && (ret_val == kCanFalse);j++) /* Maximum mailbox size pluss 8 byte header */ /* COV_CAN_HARDWARE_FAILURE */ /* FETA_CAN_LL03 */
# else
          for(j=0u; (j <  4u) && (ret_val == kCanFalse);j++)  /* 16 byte per mbox */ /* COV_CAN_HARDWARE_FAILURE */ /* FETA_CAN_LL03 */
# endif
          {
            p2Shm[j] = CanMemCheckValues32bit[i];  /* SBSW_CAN_LL_103 */ /* T0/R0 */
...
V_DEF_FUNC(CAN_LOCAL_INLINE, vuint8, STATIC_CODE) CanLL_InitPowerOn(void)
{
  vuint8 ret;
#if ( CAN_SAFE_BSW == STD_ON )
  if (CanLL_CheckGeneratedData() == kCanFailed) /* CM_CAN_LL_112 */ /* PRQA S 2992,2996,2742 */ /* MD_Can_ConstValue,MD_Can_ConstValue */ /* COV_CAN_GENDATA_FAILURE */
  {
    ret = kCanFailed; /* PRQA S 2880 */ /*  MD_MSR_Unreachable */
  }
  else
#endif
  {
...
#if defined (C_ENABLE_UPDATE_BASE_ADDRESS)
  CanLL_InitVirtualAddr(CAN_CHANNEL_CANPARA_ONLY);
#endif
  Can_GetCanBusOffNotification(canHwChannel) = 0; /* SBSW_CAN_LL_112 */ 
#if defined ( CAN_ENABLE_SECURITY_EVENT_REPORTING )
  Can_GetPSRValue(canHwChannel) = 0;              /* SBSW_CAN_LL_112 */
#endif
  Can_GetCanTXBRP(canHwChannel) = 0;              /* SBSW_CAN_LL_112 */ 
  Can_GetCanIntEnable(canHwChannel) = 0;          /* SBSW_CAN_LL_112 */ 
  Can_GetCanPrevMode(canHwChannel)  = 0xFF;       /* SBSW_CAN_LL_112 */ 
  Can_GetCanLastState(canHwChannel) = 0xFF;       /* SBSW_CAN_LL_112 */ 
  return(kCanOk);
}
/* CODE CATEGORY 4 END */
...
# if defined(C_ENABLE_TX_HW_FIFO)
    if(Can_GetMailboxType(txPara->mailboxHandle) == CAN_TX_BASICCAN_FIFO_TYPE) /* COV_CAN_GENDATA_NOT_IN_ALL_CONFIG */
    {
      txPara->lpTXB = &ShmTXB(CanGetTxHwFifoPutIndex(canHwChannel)); /* PRQA S 0303,4391 */ /* MD_Can_HwAccess,MD_Can_IntegerCast */ /* SBSW_CAN_LL_113 */
    }
    else
# endif
    {
  ...
  /* #20 check boundaries */
# if( CAN_SAFE_BSW == STD_ON ) 
  if( ((vuint32)txPara->lpTXB < (vuint32)Can_GetStartAdrOfShmAdr(canHwChannel)) || ((vuint32)&ShmTXB(txPara->hwObjHandle +1u) >= (vuint32)Can_GetStopAdrOfShmAdr(canHwChannel) ) ) /* PRQA S 0303,4391 */ /* MD_Can_HwAccess,MD_Can_IntegerCast */ /* CM_CAN_LL_103 */  /* COV_CAN_GENDATA_FAILURE */
  { /* SilentBSW check */
    Can_CallDetReportError(CAN_WRITE_ID, CAN_E_PARAM_HANDLE);
    txPara->lpTXB = (CanElmType*)Can_GetStartAdrOfShmAdr(canHwChannel); /* assure no out of bounds access */ /* PRQA S 0303 */ /* MD_Can_HwAccess */  /* SBSW_CAN_LL_113 */
  }
# endif
  CAN_CHANNEL_DUMMY_STATEMENT; /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */
}
...
#  endif
  ...
  curByte = 0u;
#  if defined (C_ENABLE_CAN_FD_FULL)
  for (curWord = 0u; (curWord << 2u) < txPara->frameLen; curWord++) /* CM_CAN_LL_105 */ /* FETA_CAN_LL04 */
#  else
...
# if defined(C_ENABLE_TX_HW_FIFO)
  if(Can_GetMailboxType(txPara->mailboxHandle) == CAN_TX_BASICCAN_FIFO_TYPE) /* COV_CAN_GENDATA_TX_NOT_IN_ALL_CONFIG */
  {
    txBRP = (vuint32)(kCanACTIVE << CanGetTxHwFifoPutIndex(canHwChannel)); /* PRQA S 0303 */ /* MD_Can_HwAccess */
  }
  else
# endif
  {
     ...
     When no Tx scan is running, the bits are reset immediately, else the bits remain set until the Tx scan process has completed. */
  {
    ...
    /* Each Tx Buffer has its own Transmission Request Pending bit. 
     ...
     Can_GetCanTXBRP(canHwChannel) |= txBRP;   /* SBSW_CAN_LL_112 */ 
  }
  return(retvalue);
}
/* CODE CATEGORY 1 END */
...
# if defined(C_ENABLE_MULTIPLEXED_TRANSMISSION) || defined(C_ENABLE_TX_HW_FIFO)
    for (;mailboxElement < Can_GetMailboxSize(taskPara->mailboxHandle); mailboxElement++) /* FETA_CAN_HL01 */
# endif
    {
      /* #10 get the corresponding Tx Buffer number to be confirmed */
      txBRP = CanLL_if_TxIsObjConfPending(CAN_CHANNEL_CANPARA_FIRST ((vuint32)(kCanACTIVE << (taskPara->hwObjHandle + mailboxElement))) ); /* PRQA S 2985 */ /* MD_Can_ConstValue */
      ...
      { /* at least one Tx buffer is pending */
        CanHL_TxConfirmationPolling(CAN_CHANNEL_AND_HW_CHANNEL_CANPARA_FIRST taskPara->mailboxHandle, mailboxElement, taskPara->hwObjHandle + mailboxElement); /* PRQA S 2985 */ /* MD_Can_ConstValue */
      }
    }
...
V_DEF_FUNC(CAN_LOCAL_INLINE, void, STATIC_CODE) CanLL_TxConfBegin(CAN_CHANNEL_AND_HW_CHANNEL_CANTYPE_FIRST CanTxConfirmationParaStructPtr txConfPara)  /* PRQA S 3673 */ /* MD_Can_NoneConstParameterPointer */
{
  ...
  Can_GetCanTXBRP(canHwChannel)     &= ((vuint32)~txBRP);  /* SBSW_CAN_LL_112 */  
  CanNestedGlobalInterruptRestore();
}
/* CODE CATEGORY 1 END */
...
V_DEF_FUNC(CAN_LOCAL_INLINE, void, STATIC_CODE) CanLL_TxConfSetTxConfStruct(CAN_CHANNEL_AND_HW_CHANNEL_CANTYPE_FIRST CanTxConfirmationParaStructPtr txConfPara) /* COV_CAN_GENDATA_TX_NOT_IN_ALL_CONFIG */
{
  ...
  {
    vuint8 i;
    for(i=0u ; i< (Can_GetMaxDataLenOfMailbox(txConfPara->mailboxHandle)/4u) ; i++) /* FETA_CAN_HL01 */
    {
  ...
  *(vuint32*)&txConfPara->tmpTXB.data[1] = CanBswap32(*(vuint32*)&txConfPara->tmpTXB.data[1]);     /* PRQA S 0303,0312,0404 */ /* MD_Can_HwAccess,MD_Can_NoneVolatilePointerCast,MD_Can_0404_LL */  /* SBSW_CAN_LL_109 */ 
  ...
  txConfPara->txStructConf->pChipData   = (CanChipDataPtr)&txConfPara->tmpTXB.data[0]; /* finally hand the byte pointer to the application */ /* SBSW_CAN_LL_109 */ 
  txConfPara->txStructConf->pChipMsgObj = (CanChipMsgPtr) &ShmTXB(txConfPara->hwObjHandle).E0;  /* PRQA S 0310,0303,4391 */ /* MD_Can_PointerCast, MD_Can_HwAccess, MD_Can_IntegerCast */  /* SBSW_CAN_LL_109 */ 
}
/* CODE CATEGORY 1 END */
#endif
...
# if defined( C_ENABLE_MULTIPLE_BASICCAN )
  if(rxBasicPara->mailboxHandle == (CAN_HL_MB_RX_BASIC_STARTINDEX(canHwChannel) + 1u) )  /* COV_CAN_GENDATA_NOT_IN_ALL_CONFIG */
  { /* point to Fifo-1 */
...
# if defined(C_ENABLE_CAN_FD_FULL)
  fElmSize = CAN_DLC2LEN(((rxBasicPara->fElm.E1) & 0x000F0000UL) >> 16u); /* SBSW_CAN_LL_109 */
  for(indexL = 0u; ( ((indexL << 2u) < fElmSize) && (indexL < (Can_GetMaxDataLenOfMailbox(rxBasicPara->mailboxHandle)/4u)) ); indexL++) /* ESCAN00113161 - Do not copy/read more than MaxDataLen */ /* FETA_CAN_LL06 */
#else
  ...
  rxBasicPara->rxStruct.pChipMsgObj = (CanChipMsgPtr)  &rxBasicPara->fElm.E0;      /* SBSW_CAN_LL_109 */ /* PRQA S 0310 */ /* MD_Can_PointerCast */
  rxBasicPara->rxStruct.pChipData   = (CanChipDataPtr) &rxBasicPara->fElm.data[0]; /* SBSW_CAN_LL_109 */ /* PRQA S 0310 */ /* MD_Can_PointerCast */
...
V_DEF_FUNC(CAN_LOCAL_INLINE, void, STATIC_CODE) CanLL_RxBasicProcessPendings(CAN_CHANNEL_AND_HW_CHANNEL_CANTYPE_FIRST CanTaskParaStructPtr taskPara) /* PRQA S 3673 */ /* MD_Can_NoneConstParameterPointer */
{
...
#if defined( C_ENABLE_MULTIPLE_BASICCAN )
  if(taskPara->mailboxHandle == (CAN_HL_MB_RX_BASIC_STARTINDEX(canHwChannel) + 1u)) /* COV_CAN_GENDATA_NOT_IN_ALL_CONFIG */
  { /* point to Fifo-1 */
  ...
  while( ((fillLevel) > 0u) ) /* FETA_CAN_LL07 */
  {
    fillLevel--;
...
# endif
  ...
  rxFullPara->rxStruct.pChipMsgObj = (CanChipMsgPtr)  &rxFullPara->bfElm;         /* SBSW_CAN_LL_109 */ /* PRQA S 0310 */ /* MD_Can_PointerCast */
  rxFullPara->rxStruct.pChipData   = (CanChipDataPtr) &rxFullPara->bfElm.data[0]; /* SBSW_CAN_LL_109 */ /* PRQA S 0310 */ /* MD_Can_PointerCast */ 
...
# if defined(C_ENABLE_CAN_FD_FULL)
  rElmSize = CAN_DLC2LEN((vuint32)((rxFullPara->bfElm.E1) & 0x000F0000UL) >> 16u); /* SBSW_CAN_LL_109 */

  /* #20 copy the data */
  for(indexL = 0u; (indexL << 2u) < rElmSize; indexL++) /* SBSW_CAN_LL_109 */ /* SBSW_CAN_LL_109 */ /* FETA_CAN_LL06 */
  {
...
/****************************************************************************
| NAME:             CanLL_GetLastErrorCode
****************************************************************************/
/*
|<DataModelStart>| CanLL_GetLastErrorCode
Relation_Context:
StatusPolling
 ...
 *
*/
V_DEF_FUNC(CAN_LOCAL_INLINE, vuint8, STATIC_CODE) CanLL_GetLastErrorCode(CAN_HW_CHANNEL_CANTYPE_ONLY)
{
...
# if defined ( C_ENABLE_CAN_FD_FULL ) || defined ( C_ENABLE_CAN_FD_USED )
  ...
  if ((Can_GetPSRValue(canHwChannel) & kCanPSR_RBRS) == kCanPSR_RBRS)
  {
   ErrorValue = (vuint8)((Can_GetPSRValue(canHwChannel) & kCanPSR_FLEC) >> 8u);
  }
  if ((ErrorValue == kCanPSR_LEC_NO_ERROR) || (ErrorValue == kCanPSR_LEC_NO_UPDATE))
  {
   /* Classic CAN Frame with errors could come before a normal FD frame (with no errors)-in this case LEC should be checked */
   ErrorValue = (vuint8)(Can_GetPSRValue(canHwChannel) & kCanPSR_LEC);
  }
# else
  ErrorValue = (vuint8)(Can_GetPSRValue(canHwChannel) & kCanPSR_LEC);
# endif
...
/**********************************************************************************************************************
 *  CanLL_ErrorStatePassiveTransitionOccurred
 *********************************************************************************************************************/
/*
|<DataModelStart>| CanLL_ErrorStatePassiveTransitionOccurred
Relation_Context:
...
V_DEF_FUNC(CAN_LOCAL_INLINE, vuint8, STATIC_CODE) CanLL_ErrorStatePassiveTransitionOccured( CAN_CHANNEL_AND_HW_CHANNEL_CANTYPE_ONLY)
{
  ...
  {
    if (CanLL_HwIsPassive( canHwChannel )) /* COV_CAN_LL_HARDWARE_BEHAVIOUR */
    {
      retVal = kCanTrue;
    }
...
# if defined(C_ENABLE_TRICORE_ERRATUM_AI_H001)
   CanLL_TriCoreErratum_AI_H001(CAN_HW_CHANNEL_CANPARA_FIRST (vuint32)(kCanIR_EP));
# else
...
switch (CanLL_GetLastErrorCode(CAN_HW_CHANNEL_CANPARA_ONLY))
 {
  case kCanPSR_LEC_Stuff_ERROR:
  /* Stuffing bits not as expected */
  CanHL_ErrorNotification(CAN_CHANNEL_AND_HW_CHANNEL_CANPARA_FIRST CAN_ERROR_CHECK_STUFFING_FAILED);
  break;

  case kCanPSR_LEC_FORM_ERROR:
  /* Violations of the fixed frame format */
  CanHL_ErrorNotification(CAN_CHANNEL_AND_HW_CHANNEL_CANPARA_FIRST CAN_ERROR_CHECK_FORM_FAILED);
  break;

  case kCanPSR_LEC_ACK_ERROR:
    /* Acknowledgement check failed */
    CanHL_ErrorNotification(CAN_CHANNEL_AND_HW_CHANNEL_CANPARA_FIRST CAN_ERROR_CHECK_ACK_FAILED);
  break;

  case kCanPSR_LEC_BIT1_ERROR:
   /* A 0 was transmitted and a 1 was read back */
   CanHL_ErrorNotification(CAN_CHANNEL_AND_HW_CHANNEL_CANPARA_FIRST CAN_ERROR_BIT_MONITORING0);
  break;

  case kCanPSR_LEC_BIT0_ERROR:
    /* A 1 was transmitted and a 0 was read back */
    CanHL_ErrorNotification(CAN_CHANNEL_AND_HW_CHANNEL_CANPARA_FIRST CAN_ERROR_BIT_MONITORING1);
  break;

  case kCanPSR_LEC_CRC_ERROR:
  /* CRC failed */
  CanHL_ErrorNotification(CAN_CHANNEL_AND_HW_CHANNEL_CANPARA_FIRST CAN_ERROR_CHECK_CRC_FAILED);
  break;
...
#if defined( CAN_ENABLE_SECURITY_EVENT_REPORTING )
  Can_GetPSRValue(canHwChannel) = Cn->PSR; /* PRQA S 0303 */ /* MD_Can_HwAccess */ /* SBSW_CAN_LL_112 */
#endif
}
/* CODE CATEGORY 2 END */
...
V_DEF_FUNC(CAN_STATIC, vuint8, STATIC_CODE) CanLL_ModeTransition(CAN_CHANNEL_AND_HW_CHANNEL_CANTYPE_FIRST vuint8 mode, vuint8 busOffRecovery, vuint8 ramCheck)
{
  ...
  Can_GetCanPrevMode(canHwChannel) = mode; /* SBSW_CAN_LL_112 */ 
  Can_GetCanLastState(canHwChannel) = ret; /* SBSW_CAN_LL_112 */
...
# endif
  ...
  CanLL_ApplCanTimerStart(kCanLoopRxFifo); /* SBSW_CAN_LL_102 */  
  while( (((Cn->RXF0S).B.FnFL) > 0UL) && (CanLL_ApplCanTimerLoop(kCanLoopRxFifo) != CAN_NOT_OK) ) /* PRQA S 1881,4558,3415,0303 */ /* MD_MSR_AutosarBoolean,MD_Can_4558_RHsideEffect,MD_Can_4558_RHsideEffect,MD_Can_HwAccess */ /* COV_CAN_GENDATA_NOT_IN_ALL_CONFIG */ /* FETA_CAN_LL01 */
  {
    Cn->RXF0A = Cn->RXF0S.B.FnGI;   /* SBSW_CAN_LL_100 */   
  }
  CanLL_ApplCanTimerEnd(kCanLoopRxFifo); /* SBSW_CAN_LL_102 */  
#if defined( C_ENABLE_MULTIPLE_BASICCAN )
  CanLL_ApplCanTimerStart(kCanLoopRxFifo); /* SBSW_CAN_LL_102 */  
  while( (((Cn->RXF1S).B.FnFL) > 0UL) && (CanLL_ApplCanTimerLoop(kCanLoopRxFifo) != CAN_NOT_OK) ) /* PRQA S 1881,4558,3415,0303 */ /* MD_MSR_AutosarBoolean,MD_Can_4558_RHsideEffect,MD_Can_4558_RHsideEffect,MD_Can_HwAccess */ /* COV_CAN_GENDATA_NOT_IN_ALL_CONFIG */ /* FETA_CAN_LL01 */
  {
...
#  if defined(C_ENABLE_USE_OS_INTERRUPT_CONTROL)  /* COV_CAN_HW_SPECIFIC */
  boolean  retval = kCanFalse;
  (void)osIsInterruptSourceEnabled(Can_GetIsrOsId(canHwChannel), &retval); /* SBSW_CAN_LL_121 */
  (void)osDisableInterruptSource(Can_GetIsrOsId(canHwChannel));
  *(localInterruptOldFlagPtr) = (vuint32) retval; /* SBSW_CAN_LL_122 */
#  else
  *(localInterruptOldFlagPtr) = (vuint32)(kCanMCMcanSRCR_Mask & CAN_READ_SRN_PROTECTED_REG32(CAN_PROTECTED_AREA_SRN,Can_GetSRN_Address(canHwChannel))); /* PRQA S 0306,0303 */ /* MD_Can_0306_HWaccess_LL,MD_Can_HwAccess */ /* SBSW_CAN_HL16 */ /* SBSW_CAN_LL_119 */
  CAN_WRITE_SRN_PROTECTED_REG32(CAN_PROTECTED_AREA_SRN,Can_GetSRN_Address(canHwChannel),((*localInterruptOldFlagPtr) & kCanMCMcanDisable_Service_Request) ); /* PRQA S 0306 */ /* MD_Can_0306_HWaccess_LL */ /* SBSW_CAN_LL_119 */ /* SBSW_CAN_LL_123 */
...
#  if defined(C_ENABLE_USE_OS_INTERRUPT_CONTROL)    /* COV_CAN_HW_SPECIFIC */
  ...
  {
    (void)osEnableInterruptSource(Can_GetIsrOsId(canHwChannel), kCanFalse);
  }
#  else
  /* #10 restore previous state (before disabling interrupts) */
  CAN_WRITE_SRN_PROTECTED_REG32(CAN_PROTECTED_AREA_SRN,Can_GetSRN_Address(canHwChannel),(vuint32)(kCanMCMcanSRCR_Mask & (CAN_READ_SRN_PROTECTED_REG32(CAN_PROTECTED_AREA_SRN,Can_GetSRN_Address(canHwChannel)) | localInterruptOldFlag)) ); /* PRQA S 0306 */ /* MD_Can_0306_HWaccess_LL */ /* SBSW_CAN_LL_119 */ /* SBSW_CAN_LL_123 */
#  endif /* C_ENABLE_USE_OS_INTERRUPT_CONTROL */
} /* CanLL_CanInterruptRestore */
/* CODE CATEGORY 1 END */
...
*/
/* CODE CATEGORY 1 START */
V_DEF_FUNC(CAN_LOCAL_INLINE, vuint8, STATIC_CODE) Can_EccInit(void)
{
  /*Local Variables*/
  vuint8 ret = kCanOk;
# if defined (C_ENABLE_HW_ECC_INIT) /* Message RAM initialization by hardware (RINIT) */
  vuint8 indx;
  volatile vuint32 dummyRead;
#  if defined( C_ENABLE_HW_LOOP_TIMER ) && !defined ( C_SINGLE_RECEIVE_CHANNEL )
  CanChannelHandle channel = 0;
#  endif

  /* #10 Disable EndInit Protection */
  CAN_ENDINIT_PROTECT_OFF();

#  if( CAN_SAFE_BSW == STD_ON ) 
  /* Amount of subsystems must be between 1 and 5 */
  if((Can_GetSizeOfCanSubSystemBaseAdr() == 0u) || (Can_GetSizeOfCanSubSystemBaseAdr() > 5u) ) /* PRQA S 3325,2992,2742 */ /* MD_Can_3325_LL,MD_Can_ConstValue,MD_Can_ConstValue */ /* COV_CAN_GENDATA_FAILURE */
  { 
    Can_CallDetReportError(CAN_INIT_ID, CAN_E_GENDATA); /* PRQA S 2880 */ /* MD_MSR_Unreachable */
  }
  else
#  endif
  {
    /* #20 Initialize the Message RAM of each active subsystem */
    for (indx=0;indx<Can_GetSizeOfCanSubSystemBaseAdr(); indx++) /* FETA_CAN_HL01 */
    {
      /* #30 Set Clock & RAM Change Enable and Change Init  */
      MCMCAN(indx)->MCR |=  (kCanMCMcan_MCR_CCCE | kCanMCMcan_MCR_CI); /* PRQA S 0303 */ /* MD_Can_HwAccess */ /* SBSW_CAN_LL_120 */
      /* #40 Wait until RBUSY is 0b */
      if ( (MCMCAN(indx)->MCR & kCanMCMcan_MCR_RBUSY) != 0u ) /* PRQA S 0303 */ /* MD_Can_HwAccess */ /* COV_CAN_HARDWARE_FAILURE */
      {
        CanLL_ApplCanTimerStart(kCanLoopRamInit); /* SBSW_CAN_LL_101 */
        do
        {
        /* Wait */
        } while( ((MCMCAN(indx)->MCR & kCanMCMcan_MCR_RBUSY) != 0u ) && (CanLL_ApplCanTimerLoop(kCanLoopRamInit) !=  CAN_NOT_OK) ); /* PRQA S 1881,3415,0303 */ /* MD_MSR_AutosarBoolean,MD_Can_4558_RHsideEffect,MD_Can_HwAccess */
        CanLL_ApplCanTimerEnd(kCanLoopRamInit); /* SBSW_CAN_LL_101 */
      }
      /* #50 Set RINIT to 0b */
      MCMCAN(indx)->MCR &= ~kCanMCMcan_MCR_RINIT; /* PRQA S 0303 */ /* MD_Can_HwAccess */ /* SBSW_CAN_LL_120 */
      /* #60 Set RINIT to 1b */
      MCMCAN(indx)->MCR |= kCanMCMcan_MCR_RINIT; /* PRQA S 0303 */ /* MD_Can_HwAccess */ /* SBSW_CAN_LL_120 */
      /* #70 Dummy Read MCR */
      dummyRead = MCMCAN(indx)->MCR; /* PRQA S 0303 */ /* MD_Can_HwAccess */
      /* #80 Wait until RBUSY is 0b */
      if ( (MCMCAN(indx)->MCR & kCanMCMcan_MCR_RBUSY) != 0u ) /* PRQA S 0303 */ /* MD_Can_HwAccess */
      {
        CanLL_ApplCanTimerStart(kCanLoopRamInit); /* SBSW_CAN_LL_101 */
        do
        {
      ...
      if ( (MCMCAN(indx)->MCR & kCanMCMcan_MCR_RBUSY) != 0u ) /* PRQA S 0303 */ /* MD_Can_HwAccess */ /* COV_CAN_HARDWARE_FAILURE */
      {
        /* Message RAM initialization Failed */
        ret = kCanFailed;
      }
      /* #90 Set RINIT to 0b */
      MCMCAN(indx)->MCR &= ~kCanMCMcan_MCR_RINIT; /* PRQA S 0303 */ /* MD_Can_HwAccess */ /* SBSW_CAN_LL_120 */
      /* #100 Reset Clock & RAM Change Enable and Change Init  */
      MCMCAN(indx)->MCR &=  ~(kCanMCMcan_MCR_CCCE | kCanMCMcan_MCR_CI); /* PRQA S 0303 */ /* MD_Can_HwAccess */ /* SBSW_CAN_LL_120 */

      CAN_DUMMY_STATEMENT(dummyRead); /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */
    }
  }
  /* #110 Enable EndInit Protection */
...
# else
  ...
  for(ch = 0; ch < kCanNumberOfChannels; ch++)
  {
    ...
    for(idx = 0; idx < length; idx++) /* FETA_CAN_LL08 */
    {
...
V_DEF_FUNC(CAN_LOCAL_INLINE, vuint32, STATIC_CODE) CanLL_if_TxIsObjConfPending( CAN_CHANNEL_CANTYPE_FIRST vuint32 txBuffer ) 
{
  ...
  if( ((Can_GetCanTXBRP(canHwChannel) & txBuffer) != 0u) && ((Cn->TXBTO & txBuffer) != 0u) ) /* PRQA S 3673,0303 */ /* MD_Can_NoneConstParameterPointer,MD_Can_HwAccess */ /* COV_CAN_HW_SPECIFIC */
...
V_DEF_FUNC(CAN_LOCAL_INLINE, void, STATIC_CODE) CanLL_TriCoreErratum_AI_H001(CAN_HW_CHANNEL_CANTYPE_FIRST vuint32 CanIrFlag)
{
  ...
  Cn->IR = (CanIrFlag); /* SBSW_CAN_LL_100 */ /* PRQA S 0303 */ /* MD_Can_HwAccess */
  /* #20 Check if the clear is neglected */
  if ( (Cn->IR & CanIrFlag) != 0u ) /* PRQA S 0303 */ /* MD_Can_HwAccess */ /* COV_CAN_HARDWARE_FAILURE */
  {
    CanLL_ApplCanTimerStart(kCanLoopIrFlag); /* SBSW_CAN_LL_101 */
    do /* FETA_CAN_LL01 */
    {
      /* #30 Repeat clearing until the flag reads zero */
      Cn->IR = (CanIrFlag); /* SBSW_CAN_LL_100 */ /* PRQA S 0303 */ /* MD_Can_HwAccess */
    } while( ((Cn->IR & CanIrFlag) != 0u ) && (CanLL_ApplCanTimerLoop(kCanLoopIrFlag) !=  CAN_NOT_OK) ); /* PRQA S 1881,3415,0303 */ /* MD_MSR_AutosarBoolean,MD_Can_4558_RHsideEffect,MD_Can_HwAccess */ /* COV_CAN_HARDWARE_FAILURE */
    CanLL_ApplCanTimerEnd(kCanLoopIrFlag); /* SBSW_CAN_LL_101 */
  }
...
V_DEF_FUNC(CAN_LOCAL_INLINE, void, STATIC_CODE) CanBoschErratum_010( CAN_HW_CHANNEL_CANTYPE_ONLY )  /* PRQA S 3673 */ /* MD_Can_NoneConstParameterPointer */
{
  ...
  { /* operating as transmitter */
    ...
    CanLL_ApplCanTimerStart(kCanLoopClockStop); /* SBSW_CAN_LL_101 */  
    ...
    CanLL_ApplCanTimerEnd(kCanLoopInit); /* SBSW_CAN_LL_101 */  
...
V_DEF_FUNC(CAN_LOCAL_INLINE, vuint8, STATIC_CODE) CanErrorHandlingBusOff(CAN_HW_CHANNEL_CANTYPE_ONLY)
{
  ...
  if( (((Cn->IR & kCanIR_BO)  != 0UL) && ((Can_GetPSRValue(canHwChannel) & kCanPSR_BO) != 0UL)) /* PRQA S 0303 */ /* MD_Can_HwAccess */
#else
  ...
  { 
    ...
    { /* BusOff status changed to BusOff OR Bit Error Uncorrected appeared */
      ...
      { 
        Can_GetCanBusOffNotification(canHwChannel) = kCanBusoff;  /* SBSW_CAN_LL_112 */ 
      }
    }
    /* #40 reset flags */
# if defined(C_ENABLE_TRICORE_ERRATUM_AI_H001)
...
#if (CAN_DEV_ERROR_REPORT == STD_ON) 
        Can_CallDetReportError(CAN_CTRBUSOFF_ID, CAN_E_MRAF); /* PRQA S 2880 */ /*  MD_MSR_Unreachable */
#endif
 ...
 *
 */
V_DEF_FUNC(CAN_LOCAL_INLINE, vuint8, STATIC_CODE) CanLL_SetAllFilter(CAN_CHANNEL_CANTYPE_FIRST CanInitParaStructPtr initPara) /* PRQA S 3673 */ /* MD_Can_NoneConstParameterPointer */
{
#if( CAN_SAFE_BSW == STD_ON ) 
...
#if defined( C_ENABLE_STD_ID ) && (kCanNumberOfStandardFilter > 0)  /* at least one std filter is available */
  initPara->pFS = &Can_GetCanSIDFE(Can_GetCanSIDFEStartIndex(canHwChannel));  /* SBSW_CAN_LL_109 */ 
  for(initPara->tmpVal = 0; initPara->tmpVal < (Can_GetCanSIDFC_LSSOfCanSIDFC(canHwChannel)); (initPara->tmpVal)++) /* SBSW_CAN_LL_109 */  /* SBSW_CAN_LL_109 */ /* COV_CAN_GENDATA_NOT_IN_ALL_CONFIG */ /* FETA_CAN_HL01 */
  { /* copy std filter elements to SHM */
# if( CAN_SAFE_BSW == STD_ON ) 
    if( ((vuint32)&ShmSIDF(initPara->stdFeCount).S0 < (vuint32)Can_GetStartAdrOfShmAdr(canHwChannel)) || ((vuint32)&ShmSIDF(initPara->stdFeCount).S0 >= (vuint32)Can_GetStopAdrOfShmAdr(canHwChannel) ) ) /* PRQA S 0303 */ /* MD_Can_HwAccess */ /* CM_CAN_LL_103 */  /* COV_CAN_GENDATA_FAILURE */
    { /* SilentBSW check */
      Can_CallDetReportError(CAN_INIT_ID, CAN_E_GENDATA);
      result = kCanFailed;
    }
...
# endif
    {
      (void)CanLL_WriteReg32(CAN_CHANNEL_CANPARA_FIRST &(ShmSIDF(initPara->stdFeCount).S0), (vuint32)(initPara->pFS[initPara->tmpVal]), kCanRegCheckMaskSMIDFE_S0, initPara);   /* PRQA S 0303 */ /* MD_Can_HwAccess */ /* SBSW_CAN_LL_103 */
...
#if defined(C_ENABLE_EXTENDED_ID) && (kCanNumberOfExtendedFilter > 0)
  for(initPara->tmpVal = 0; initPara->tmpVal < (Can_GetCanXIDFC_LSEOfCanXIDFC(canHwChannel)); (initPara->tmpVal)++) /* SBSW_CAN_LL_109 */  /* SBSW_CAN_LL_109 */ /* FETA_CAN_HL01 */
  { /* copy ext filter elements to SHM */
# if( CAN_SAFE_BSW == STD_ON ) 
    if( ((vuint32)&ShmXIDF(initPara->extFeCount).F0 < (vuint32)Can_GetStartAdrOfShmAdr(canHwChannel)) || ((vuint32)&ShmXIDF(initPara->extFeCount).F0 >= (vuint32)Can_GetStopAdrOfShmAdr(canHwChannel) ) ) /* PRQA S 0303 */ /* MD_Can_HwAccess */ /* CM_CAN_LL_103 */ /* COV_CAN_GENDATA_FAILURE */
    { /* SilentBSW check */
      Can_CallDetReportError(CAN_INIT_ID, CAN_E_GENDATA);
      result = kCanFailed;
    }
...
# endif
    {
      (void)CanLL_WriteReg32(CAN_CHANNEL_CANPARA_FIRST &(ShmXIDF(initPara->extFeCount).F0), (vuint32)(Can_GetCanXIDFEEIDFE_0OfCanXIDFE(Can_GetCanXIDFEStartIndex(canHwChannel) + initPara->tmpVal)), kCanRegCheckMaskEMIDFE_F0, initPara);  /* SBSW_CAN_LL_103 */ /* PRQA S 0303 */ /* MD_Can_HwAccess */
      (void)CanLL_WriteReg32(CAN_CHANNEL_CANPARA_FIRST &(ShmXIDF(initPara->extFeCount).F1), (vuint32)(Can_GetCanXIDFEEIDFE_1OfCanXIDFE(Can_GetCanXIDFEStartIndex(canHwChannel) + initPara->tmpVal)), kCanRegCheckMaskEMIDFE_F1, initPara);  /* SBSW_CAN_LL_103 */ /* PRQA S 0303 */ /* MD_Can_HwAccess */
...
V_DEF_FUNC(CAN_LOCAL_INLINE, vuint8, STATIC_CODE) CanLL_SetAllRegister(CAN_CHANNEL_CANTYPE_FIRST CanInitParaStructPtr initPara)
{
...
#if defined(C_ENABLE_STD_ID) 
  CAN_DUMMY_STATEMENT(initPara->stdFeCount);  /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */ /* SBSW_CAN_LL_109 */ 
#endif
#if defined(C_ENABLE_EXTENDED_ID) 
  CAN_DUMMY_STATEMENT(initPara->extFeCount);  /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */ /* SBSW_CAN_LL_109 */ 
#endif
      ...
      requests have been completed and the CAN bus reached idle. */
  {
    ...
    CanLL_ApplCanTimerStart(kCanLoopClockStop); /* SBSW_CAN_LL_101 */
    while( ((Cn->CCCR & kCanCCCR_CSA) == 0UL) && (CanLL_ApplCanTimerLoop(kCanLoopClockStop) !=  CAN_NOT_OK) ) /* PRQA S 1881,4558,3415,0303 */ /* MD_MSR_AutosarBoolean,MD_Can_4558_RHsideEffect,MD_Can_4558_RHsideEffect,MD_Can_HwAccess */ /* COV_CAN_HW_SPECIFIC */ /* FETA_CAN_LL01 */
  ...
  CanLL_ApplCanTimerStart(kCanLoopInit); /* SBSW_CAN_LL_101 */  
  while( ((Cn->CCCR & kCanCCCR_INIT) == 0UL) && (CanLL_ApplCanTimerLoop(kCanLoopInit) !=  CAN_NOT_OK) ) /* PRQA S 1881,4558,3415,0303 */ /* MD_MSR_AutosarBoolean,MD_Can_4558_RHsideEffect,MD_Can_4558_RHsideEffect,MD_Can_HwAccess */ /* COV_CAN_HW_SPECIFIC */ /* FETA_CAN_LL01 */
  { /* Assure that INIT has been accepted. */ }
  CanLL_ApplCanTimerEnd(kCanLoopInit);  /* SBSW_CAN_LL_101 */  
#endif      
  ...
  CanLL_ApplCanTimerStart(kCanLoopInit); /* SBSW_CAN_LL_101 */  
  while( ((Cn->CCCR & kCanCCCR_CCE) == 0UL) && (CanLL_ApplCanTimerLoop(kCanLoopInit) !=  CAN_NOT_OK) ) /* PRQA S 1881,4558,3415,0303 */ /* MD_MSR_AutosarBoolean,MD_Can_4558_RHsideEffect,MD_Can_4558_RHsideEffect,MD_Can_HwAccess */ /* COV_CAN_HW_SPECIFIC */ /* FETA_CAN_LL01 */
  { /* Assure that the previous value has been accepted. */ }
  CanLL_ApplCanTimerEnd( kCanLoopInit ); /* SBSW_CAN_LL_101 */  
#endif
...
# endif
  ...
  (void)CanLL_WriteReg32(CAN_CHANNEL_CANPARA_FIRST &(Cn->CCCR), (vuint32)LocalRegValue, kCan_ALL_ONE, initPara);  /* PRQA S 0303 */ /* MD_Can_HwAccess */ /* SBSW_CAN_LL_100 */ 
  ...
  /* Start value of the Message RAM Watchdog Counter. With the reset value of '0' the counter is disabled. 
     ...
     The RAM Watchdog Counter is clocked by the Host clock. */
  (void)CanLL_WriteReg32(CAN_CHANNEL_CANPARA_FIRST &(Cn->RWD), (vuint32)kCan_RWD, kCanRegCheckMaskRWD, initPara);  /* SBSW_CAN_LL_100 */    /* PRQA S 0303 */ /* MD_Can_HwAccess */
...
#  error ">>> Note: With CAN FD an external counter is required for timestamp generation (TSCC.TSS = "10") <<<"
#endif
  (void)CanLL_WriteReg32(CAN_CHANNEL_CANPARA_FIRST &(Cn->TSCC), (vuint32)(kCan_TSCC_TCP|kCan_TSCC_TSS), kCanRegCheckMaskTSCC, initPara); /* PRQA S 0303 */ /* MD_Can_HwAccess */ /* SBSW_CAN_LL_100 */ /* '0' = keep the counter quiet - otherwise use (kCan_TSCC_TCP|kCan_TSCC_TSS) */
...
#if ( CAN_BOSCH_ERRATUM_012 == STD_ON ) /* COV_CAN_ERRATUM_OFF */
  {
    (void)CanLL_WriteReg32(CAN_CHANNEL_CANPARA_FIRST &(Cn->TOCC), (vuint32)( (vuint32)(kCan_TOCC_TOP<<16) | kCan_TOCC_TOS | kCan_TOCC_ETOC ), kCanRegCheckMaskTOCC, initPara); /* SBSW_CAN_LL_100 */    /* PRQA S 0303 */ /* MD_Can_HwAccess */
  }
#else
  (void)CanLL_WriteReg32(CAN_CHANNEL_CANPARA_FIRST &(Cn->TOCC), (vuint32)(0UL), kCanRegCheckMaskTOCC, initPara);  /* SBSW_CAN_LL_100 */     /* '0' = keep the counter quiet */ /* PRQA S 0303 */ /* MD_Can_HwAccess */
#endif
  ...
  CAN_DUMMY_STATEMENT(initPara->tmpVal);  /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */  /* SBSW_CAN_LL_109 */ 
  ...
  (void)CanLL_WriteReg32(CAN_CHANNEL_CANPARA_FIRST &(Cn->RXBC), (vuint32)Can_GetCanRXBC(canHwChannel), kCanRegCheckMaskRXBC, initPara);   /* SBSW_CAN_LL_100 */    /* PRQA S 0303 */ /* MD_Can_HwAccess */  /* Rx Buffer Start Address (RBSA) configures the start address of the Rx Buffers section in the Message RAM */
#if defined(C_ENABLE_CAN_FD_FULL)
  ...
  (void)CanLL_WriteReg32(CAN_CHANNEL_CANPARA_FIRST &(Cn->RXESC.R), (vuint32)LocalRegValue, kCanRegCheckMaskRXESC, initPara);    /* SBSW_CAN_LL_100 */    /* PRQA S 0303 */ /* MD_Can_HwAccess */
#else
  (void)CanLL_WriteReg32(CAN_CHANNEL_CANPARA_FIRST &(Cn->RXESC.R), (vuint32)(0x00000000UL), kCanRegCheckMaskRXESC, initPara);    /* SBSW_CAN_LL_100 */    /* PRQA S 0303 */ /* MD_Can_HwAccess */ /* Reset to default value */
#endif

  /* #60 support (Multiple) BasicCAN */
#if defined( C_ENABLE_RX_BASICCAN_OBJECTS )
  /* Rx FIFO 0 Configuration (RXF0C) */
  if(Can_GetRxBasicHandleMax(canHwChannel) > 0u) /* COV_CAN_GENDATA_NOT_IN_ALL_CONFIG */ /* PRQA S 2741 */ /* MD_Can_ConstValue */
  {
    (void)CanLL_WriteReg32(CAN_CHANNEL_CANPARA_FIRST &(Cn->RXF0C.R), (vuint32)(Can_GetCanRXF0C(canHwChannel)), kCanRegCheckMaskRXF0C, initPara);    /* SBSW_CAN_LL_100 */  /* PRQA S 0303 */ /* MD_Can_HwAccess */  /* Using default value kCanRXFnOM_BLOCK */
  }
  else
  { 
    (void)CanLL_WriteReg32(CAN_CHANNEL_CANPARA_FIRST &(Cn->RXF0C.R), (vuint32)(0x00000000UL), kCanRegCheckMaskRXF0C, initPara);   /* SBSW_CAN_LL_100 */   /* PRQA S 0303,2880 */ /* MD_Can_HwAccess,MD_Can_ConstValue */
  }
...
# if defined( C_ENABLE_MULTIPLE_BASICCAN )
  ...
  if(Can_GetRxBasicHandleMax(canHwChannel) > 1u) /* COV_CAN_GENDATA_NOT_IN_ALL_CONFIG */ /* PRQA S 2741 */ /* MD_Can_ConstValue */
  {
    (void)CanLL_WriteReg32(CAN_CHANNEL_CANPARA_FIRST &(Cn->RXF1C.R), (vuint32)(Can_GetCanRXF1C(canHwChannel)), kCanRegCheckMaskRXF1C, initPara); /* SBSW_CAN_LL_100 */ /* PRQA S 0303 */ /* MD_Can_HwAccess */ /* Using default value kCanRXFnOM_BLOCK */
  }
  else
  { 
    (void)CanLL_WriteReg32(CAN_CHANNEL_CANPARA_FIRST &(Cn->RXF1C.R), (vuint32)(0x00000000UL), kCanRegCheckMaskRXF1C, initPara);   /* SBSW_CAN_LL_100 */ /* PRQA S 0303, 2880 */ /* MD_Can_HwAccess, MD_MSR_Unreachable */
  }
# else
  (void)CanLL_WriteReg32(CAN_CHANNEL_CANPARA_FIRST &(Cn->RXF1C.R), (vuint32)(0x00000000UL), kCanRegCheckMaskRXF1C, initPara);    /* SBSW_CAN_LL_100 */ /* PRQA S 0303 */ /* MD_Can_HwAccess */ /* Fifo 1 not supported */
# endif
  ...
  (void)CanLL_WriteReg32(CAN_CHANNEL_CANPARA_FIRST &(Cn->RXF0C.R), (vuint32)(0x00000000UL), kCanRegCheckMaskRXF0C, initPara);    /* SBSW_CAN_LL_100 */   /* Fifo 0 not supported */  /* PRQA S 0303 */ /* MD_Can_HwAccess */
  (void)CanLL_WriteReg32(CAN_CHANNEL_CANPARA_FIRST &(Cn->RXF1C.R), (vuint32)(0x00000000UL), kCanRegCheckMaskRXF1C, initPara);    /* SBSW_CAN_LL_100 */   /* Fifo 1 not supported */  /* PRQA S 0303 */ /* MD_Can_HwAccess */
#endif /* C_ENABLE_RX_BASICCAN_OBJECTS */
  
  /* Tx Buffer Configuration (TXBC): */
  (void)CanLL_WriteReg32(CAN_CHANNEL_CANPARA_FIRST &(Cn->TXBC), (vuint32)(Can_GetCanTXBC(canHwChannel)), kCanRegCheckMaskTXBC, initPara);   /* SBSW_CAN_LL_100 */    /* PRQA S 0303 */ /* MD_Can_HwAccess */
  /* #70 support Tx side */
  ...
  (void)CanLL_WriteReg32(CAN_CHANNEL_CANPARA_FIRST &(Cn->TXESC), (vuint32)(Can_GetTBDSOfShmElementSize(canHwChannel)), kCanRegCheckMaskTXESC, initPara);   /* SBSW_CAN_LL_100 */    /* PRQA S 0303 */ /* MD_Can_HwAccess */
...
#else
  (void)CanLL_WriteReg32(CAN_CHANNEL_CANPARA_FIRST &(Cn->TXESC), (vuint32)(0x00000000UL), kCanRegCheckMaskTXESC, initPara);   /* SBSW_CAN_LL_100 */    /* 8 data bytes */ /* PRQA S 0303 */ /* MD_Can_HwAccess */
#endif

  /* Tx Event FIFO Configuration (TXEFC) */
  (void)CanLL_WriteReg32(CAN_CHANNEL_CANPARA_FIRST &(Cn->TXEFC), (vuint32)(0x00000000UL), kCanRegCheckMaskTXEFC, initPara);   /* SBSW_CAN_LL_100 */    /* No Tx Event Fifo supported */ /* PRQA S 0303 */ /* MD_Can_HwAccess */
  ...
  {
# if defined( C_ENABLE_INDIVIDUAL_POLLING )
    if(Can_IsMailboxIndivPolling(CAN_HL_MB_RX_BASIC_STARTINDEX(canHwChannel)) == kCanFalse) /* PRQA S 1881 */ /* MD_MSR_AutosarBoolean */ /* COV_CAN_GENDATA_NOT_IN_ALL_CONFIG */
...
# if defined( C_ENABLE_MULTIPLE_BASICCAN )
  ...
  {
#  if defined( C_ENABLE_INDIVIDUAL_POLLING )
    if(Can_IsMailboxIndivPolling(CAN_HL_MB_RX_BASIC_STARTINDEX(canHwChannel) + 1u) == kCanFalse) /* PRQA S 1881 */ /* MD_MSR_AutosarBoolean */ /* COV_CAN_GENDATA_NOT_IN_ALL_CONFIG */
#  endif /* C_ENABLE_INDIVIDUAL_POLLING */
    {
...
#  if( (!defined( C_ENABLE_RX_FULLCAN_POLLING )) || (defined( C_ENABLE_INDIVIDUAL_POLLING )) )
  ...
  /* After the last word of a matching Rx message has been written to the Message RAM the NDF within NDAT1,2 is set. 
     ...
     - an Rx FIFO, or
     - the message may be rejected. */
# endif /* C_ENABLE_RX_FULLCAN_POLLING || C_ENABLE_INDIVIDUAL_POLLING */
#endif /* C_ENABLE_RX_FULLCAN_OBJECTS */
...
# endif
  ...
  if(Can_IsIsTTCan(canHwChannel)) /* TTCAN */ /* COV_CAN_HW_SPECIFIC */
  {
    ...
    (void)CanLL_WriteReg32(CAN_CHANNEL_CANPARA_FIRST &(Cn->TTOCF), (vuint32)(0x00000000UL), kCanRegCheckMaskTTOCF, initPara);   /* SBSW_CAN_LL_100 */    /* PRQA S 0303 */ /* MD_Can_HwAccess */
  }
#endif
 ...
 *
 */
V_DEF_FUNC(CAN_LOCAL_INLINE, void, STATIC_CODE) CanLL_SetRegisterMcMcan(CAN_CHANNEL_CANTYPE_FIRST CanInitParaStructPtr localInfo)
{
  /* #10 Set Interrupt routing */
  (void)CanLL_WriteReg32(CAN_CHANNEL_CANPARA_FIRST &(Cn->GRINT1),(vuint32)(Can_GetINT_RoutingGroup(canHwChannel)),kCanRegCheckMaskMCMCAN_GRINT, localInfo); /* select Interrupt output line INT_O0 */ /* SBSW_CAN_LL_100 */ /* SBSW_CAN_HL36 */ /* PRQA S 0303 */ /* MD_Can_HwAccess */
  (void)CanLL_WriteReg32(CAN_CHANNEL_CANPARA_FIRST &(Cn->GRINT2),(vuint32)(Can_GetINT_RoutingGroup(canHwChannel)),kCanRegCheckMaskMCMCAN_GRINT, localInfo); /* for each CAN channel */ /* SBSW_CAN_LL_100 */ /* SBSW_CAN_HL36 */ /* PRQA S 0303 */ /* MD_Can_HwAccess */
  /* Node Timer Clock Control Register */
  (void)CanLL_WriteReg32(CAN_CHANNEL_CANPARA_FIRST &(Cn->NTCCR),(vuint32)(0x00000000UL),kCanRegCheckMaskMCMCANCH_NTCCR, localInfo); /* SBSW_CAN_LL_100 */ /* SBSW_CAN_HL36 */ /* PRQA S 0303 */ /* MD_Can_HwAccess */
  /* Node Timer X Transmit Trigger Register */
  (void)CanLL_WriteReg32(CAN_CHANNEL_CANPARA_FIRST &(Cn->NTATTR),(vuint32)(0x00000000UL),kCanRegCheckMaskMCMCANCH_NTXTTR, localInfo); /* SBSW_CAN_LL_100 */ /* SBSW_CAN_HL36 */ /* PRQA S 0303 */ /* MD_Can_HwAccess */
  (void)CanLL_WriteReg32(CAN_CHANNEL_CANPARA_FIRST &(Cn->NTBTTR),(vuint32)(0x00000000UL),kCanRegCheckMaskMCMCANCH_NTXTTR, localInfo); /* SBSW_CAN_LL_100 */ /* SBSW_CAN_HL36 */ /* PRQA S 0303 */ /* MD_Can_HwAccess */
  (void)CanLL_WriteReg32(CAN_CHANNEL_CANPARA_FIRST &(Cn->NTCTTR),(vuint32)(0x00000000UL),kCanRegCheckMaskMCMCANCH_NTXTTR, localInfo); /* SBSW_CAN_LL_100 */ /* SBSW_CAN_HL36 */ /* PRQA S 0303 */ /* MD_Can_HwAccess */
  /* Node Timer Receive Timeout Register */
  (void)CanLL_WriteReg32(CAN_CHANNEL_CANPARA_FIRST &(Cn->NTRTR),(vuint32)(0x00000000UL),kCanRegCheckMaskMCMCANCH_NTRTR, localInfo); /* SBSW_CAN_LL_100 */ /* SBSW_CAN_HL36 */ /* PRQA S 0303 */ /* MD_Can_HwAccess */
 /* Node Port Control Register */
  (void)CanLL_WriteReg32(CAN_CHANNEL_CANPARA_FIRST &(Cn->NPCR),(vuint32)(kCanMCMcan_NPCR_DELE | Can_GetRxSelectionOfControllerConfig(canHwChannel)),kCanRegCheckMaskMCMCAN_NPCR, localInfo); /* SBSW_CAN_LL_100 */ /* SBSW_CAN_HL36 */ /* PRQA S 0303 */ /* MD_Can_HwAccess */
...
#   if defined( C_ENABLE_ISR_MANDATORY )
  /* Power On */
  if( (Can_GetLogStatus(channel) & kCanHwIsInit) != kCanHwIsInit ){
    /* #20 Enable Service Request */
    /* [0..7] ISRPrio, [10] Service Request enable, [11..12] TOS=TriCore interrupt: 0=CPU0, 1=DMA, 2=CPU1, 3=CPU2, 4=CPU3, 5=CPU4, 6=CPU5 */
    CAN_WRITE_SRN_PROTECTED_REG32(CAN_PROTECTED_AREA_SRN,Can_GetSRN_Address(canHwChannel),(vuint32)( (vuint32)kCanMCMcanSRCR_Mask & ( ((vuint32)Can_GetINT_Priority(canHwChannel) | (((vuint32)1u) << 10u)) | (((vuint32)Can_GetkCanUsedCore()) << 11u) ) )); /* PRQA S 0306,2986,2985 */ /* MD_Can_0306_HWaccess_LL,MD_Can_ConstValue,MD_Can_ConstValue */ /* SBSW_CAN_LL_119 */
  }
#   endif
#  endif
# endif
...
# if defined (C_ENABLE_UPDATE_BASE_ADDRESS)
  if ((Can_GetSizeOfShmAdr() != kCanNumberOfChannels) || (Can_GetSizeOfVirtualShmAdr() != kCanNumberOfChannels)) /* CM_CAN_LL_112 */ /* PRQA S 2992,2742 */ /* MD_Can_ConstValue,MD_Can_ConstValue */ /* COV_CAN_GENDATA_FAILURE */
# else
  if (Can_GetSizeOfShmAdr() != kCanNumberOfChannels) /* CM_CAN_LL_112 */ /* PRQA S 2992,2742 */ /* MD_Can_ConstValue,MD_Can_ConstValue */ /* COV_CAN_GENDATA_FAILURE */
# endif
    {
    Can_CallDetReportError(CAN_INIT_ID, CAN_E_GENDATA); /* PRQA S 2880 */ /* MD_MSR_Unreachable */
    ret = kCanFailed;
...
/* CODE CATEGORY 4 END */
#endif /* End C_ENABLE_REGEXT_MCANSS */


/* CODE CATEGORY 4 START */
/****************************************************************************
| NAME:             CanLL_SetGlobalRegisterMcMcan
****************************************************************************/
/*!
 * Internal comment removed.
 *
 ...
 *
 */
V_DEF_FUNC(CAN_LOCAL_INLINE, void, STATIC_CODE) CanLL_SetGlobalRegisterMcMcan(void)
{
  /*Local Variables*/
  vuint8 indx;
  ...
  /* #20 Set Global Sybsystem Specific Functionality */
#  if( CAN_SAFE_BSW == STD_ON ) 
  /* Amount of subsystems must be between 1 and 5 */
  if((Can_GetSizeOfCanSubSystemBaseAdr() == 0u) || (Can_GetSizeOfCanSubSystemBaseAdr() > 5u) ) /* PRQA S 3325,2992,2742 */ /* MD_Can_3325_LL,MD_Can_ConstValue,MD_Can_ConstValue */ /* COV_CAN_GENDATA_FAILURE */
  { 
...
#  endif
  {
    /* #30 Get Number of Active Subsystems */
    for (indx=0;indx<Can_GetSizeOfCanSubSystemBaseAdr(); indx++) /* FETA_CAN_HL01 */
    {
      CAN_WRITE_PROTECTED_REG32(CAN_PROTECTED_AREA_SUB(indx),&(MCMCAN(indx)->CLC),0); /* PRQA S 0303 */ /* MD_Can_HwAccess */ /* SBSW_CAN_LL_120 */
      MCMCAN(indx)->MCR       = kCanMCMcanSetMCR_prog_sequence_0; /* PRQA S 0303 */ /* MD_Can_HwAccess */ /* SBSW_CAN_LL_120 */
      MCMCAN(indx)->MCR       = kCanMCMcanSetMCR_prog_sequence_1; /* PRQA S 0303 */ /* MD_Can_HwAccess */ /* SBSW_CAN_LL_120 */
      MCMCAN(indx)->MCR       = kCanMCMcanSetMCR_prog_sequence_2; /* PRQA S 0303 */ /* MD_Can_HwAccess */ /* SBSW_CAN_LL_120 */
      MCMCAN(indx)->BUFADR    = 0x00000000UL; /* PRQA S 0303 */ /* MD_Can_HwAccess */ /* SBSW_CAN_LL_120 */
      MCMCAN(indx)->MECR      = 0x00000000UL; /* PRQA S 0303 */ /* MD_Can_HwAccess */ /* SBSW_CAN_LL_120 */
      MCMCAN(indx)->MESTAT    = 0x00000000UL; /* PRQA S 0303 */ /* MD_Can_HwAccess */ /* SBSW_CAN_LL_120 */
    }
  }

  /* #40 Enable EndInit Protection */
  CAN_ENDINIT_PROTECT_ON();
} /* CanLL_SetGlobalRegisterMcMcan */
/* CODE CATEGORY 4 END */
...
# endif
    {
      ...
      { /* hwObjHandle for RxFifo0 is 0 */
        CanHL_BasicCanMsgReceived(CAN_HW_CHANNEL_CANPARA_FIRST (CAN_HL_MB_RX_BASIC_STARTINDEX(canHwChannel)), 0u ); 
      }
      CanLL_ApplCanTimerEnd(kCanLoopRxFifo); /* SBSW_CAN_LL_102 */  
    }
  }
...
#  endif
    {
      CanLL_ApplCanTimerStart(kCanLoopRxFifo); /* SBSW_CAN_LL_102 */  
      while( (((Cn->RXF1S).B.FnFL) > 0UL) && (CanLL_ApplCanTimerLoop(kCanLoopRxFifo) !=  CAN_NOT_OK) ) /* PRQA S 1881,4558,3415,0303 */ /* MD_MSR_AutosarBoolean,MD_Can_4558_RHsideEffect,MD_Can_4558_RHsideEffect,MD_Can_HwAccess */ /* COV_CAN_HW_SPECIFIC */ /* FETA_CAN_LL01 */
      {
        /* Increment the MailboxHandle. There is an  additional mailbox object for FIFO-1. hwObjHandle for RxFifo-1 is 1 */
        CanHL_BasicCanMsgReceived(CAN_HW_CHANNEL_CANPARA_FIRST (CAN_HL_MB_RX_BASIC_STARTINDEX(canHwChannel) + 1u), 1u ); 
      }
      CanLL_ApplCanTimerEnd(kCanLoopRxFifo); /* SBSW_CAN_LL_102 */  
    }
  }
...
# endif
    ...
    idx = 0u;
    hwObjHandle = 0u;
    while( ((ndata[0] | ndata[1]) > 0UL) && (idx < 2u) ) /* CM_CAN_LL_107 */  /* COV_CAN_GENDATA_NOT_IN_ALL_CONFIG */ /* FETA_CAN_LL09 */
    { /* at least one buffer not read yet AND first sweep */
      shiftVal = (vuint32)(0x00000001UL << ndf);
      if((ndata[idx] & shiftVal) != 0UL)
      { /* detected NDF */
#  if defined( C_ENABLE_INDIVIDUAL_POLLING )
        if(Can_IsMailboxIndivPolling(CAN_HL_MB_RX_FULL_STARTINDEX(canHwChannel) + hwObjHandle))  /* COV_CAN_GENDATA_NOT_IN_ALL_CONFIG */
        { 
          /* poll this object later on during the next read task cycle */ 
        }
        else
#  endif
        { /* Attention:
             While an Rx Buffers NDF is set the belonging ID Filter will not match, causing the acceptance filtering to continue. 
             Following ID Filters may cause the received message to be stored into another Rx Buffer or into an Rx FIFO or the message may be rejected. */
          CanHL_FullCanMsgReceived(CAN_HW_CHANNEL_CANPARA_FIRST ((CAN_HL_MB_RX_FULL_STARTINDEX(canHwChannel)) + hwObjHandle), hwObjHandle);
        }
...
#  if defined ( C_ENABLE_CAN_FD_FULL ) || defined ( C_ENABLE_CAN_FD_USED )    
       /* Clear the PED flag */
#   if defined(C_ENABLE_TRICORE_ERRATUM_AI_H001)
    CanLL_TriCoreErratum_AI_H001(CAN_HW_CHANNEL_CANPARA_FIRST (vuint32)(kCanIR_PED));
#   else
...
#  endif
      /* Clear the PEA flag */
#  if defined(C_ENABLE_TRICORE_ERRATUM_AI_H001)
   CanLL_TriCoreErratum_AI_H001(CAN_HW_CHANNEL_CANPARA_FIRST (vuint32)(kCanIR_PEA));
#  else
...
# endif
    ...
    while ( (hwObjHdl < kCanTXBC_NDTB) && (Can_GetCanTXBRP(canHwChannel) > 0UL) ) /* FETA_CAN_LL10 */
...
#  if defined( C_ENABLE_MULTIPLEXED_TRANSMISSION ) || defined(C_ENABLE_TX_HW_FIFO)
      mailboxElement = Can_GetMailboxElementOfMemorySectionObjects(Can_GetMemorySectionStartOfMemorySectionInfo(canHwChannel) + hwObjHdl);
#  endif
...
# endif
        {
          CanHL_TxConfirmation(CAN_CHANNEL_CANPARA_FIRST mboxHdl, mailboxElement, hwObjHdl);
        }
      }
      hwObjHdl++;
      bPos <<= 1;
...
#  endif /* CAN_MCAN_REVISION */
  { /* PXE and ACT is reset on read.
    ...
    CanLL_ApplCanTimerStart(kCanLoopInit); /* SBSW_CAN_LL_102 */  
    ...
    { /* Assure that INIT has been accepted */ }
    CanLL_ApplCanTimerEnd(kCanLoopInit);  /* SBSW_CAN_LL_102 */  
    Cn->CCCR &= (vuint32)(~kCanCCCR_INIT);  /* SBSW_CAN_LL_100 */   
  }
...
V_DEF_FUNC(CAN_LOCAL_INLINE, vuint8, STATIC_CODE) CanLL_StartSilentRequest(CAN_HW_CHANNEL_CANTYPE_ONLY)
{
  ...
  else
  {
    ...
    /* #30 check current state: if already reached then avoid asynchronous all */
    CanLL_ApplCanTimerStart(kCanLoopInit); /* SBSW_CAN_LL_101 */  
    while( ((Cn->CCCR & (vuint32)(kCanCCCR_MON | kCanCCCR_ASM | kCanCCCR_INIT)) != (kCanCCCR_MON | kCanCCCR_ASM)) && (CanLL_ApplCanTimerLoop(kCanLoopInit) !=  CAN_NOT_OK) ) /* PRQA S 1881,4558,3415,0303 */ /* MD_MSR_AutosarBoolean,MD_Can_4558_RHsideEffect,MD_Can_4558_RHsideEffect,MD_Can_HwAccess */ /* COV_CAN_HW_SPECIFIC */ /* FETA_CAN_LL01 */
...
#if defined(C_ENABLE_CANCEL_SUPPORT_API)
      /* #140 Remember SEND STATE which may be modified due to new transmit call in confirmation */
...
# endif
  {
    ...
    for (baudrateIndex = Can_GetInitObjectStartIndex(channel); baudrateIndex < Can_GetInitObjectStartIndex(channel + 1u); baudrateIndex++) /* FETA_CAN_HL01 */
...
# if defined(C_ENABLE_TX_POLLING) /* COV_CAN_TX_POLLING TX */ /* always true when no hw-cancel is supported */
          ...
          { /* #110 is at least one mailbox pending */
            for ( taskPara.mailboxHandle = Can_GetTxBasicHandleStart(canHwChannel); taskPara.mailboxHandle < Can_GetTxBasicHandleStop(canHwChannel); taskPara.mailboxHandle++ ) /* PRQA S 2994,2996 */ /* MD_Can_ConstValue */ /* FETA_CAN_HL01 */
...
#  if defined(C_ENABLE_TX_POLLING) /* COV_CAN_TX_POLLING TX */ /* always true when no hw-cancel is supported */
          ...
          { /* #210 is at least one mailbox pending */
            for ( taskPara.mailboxHandle = Can_GetTxFullHandleStart(canHwChannel); taskPara.mailboxHandle < Can_GetTxFullHandleStop(canHwChannel); taskPara.mailboxHandle++ ) /* PRQA S 2994,2996 */ /* MD_Can_ConstValue */ /* FETA_CAN_HL01 */
...
# if defined(C_ENABLE_MULTIPLEXED_TRANSMISSION) || defined(C_ENABLE_TX_HW_FIFO) /* \trace SPEC-1677 */
        for (; txCancellationPara.mailboxElement < Can_GetMailboxSize(Hth); txCancellationPara.mailboxElement++, txCancellationPara.activeSendObject++) /* PRQA S 2463,3387,3418 */ /* MD_Can_HL_AdditionalExpressionInForLoop,MD_Can_HL_UsingIncOrDec,MD_Can_IncompleteForLoop */ /* FETA_CAN_HL05 */
# endif
...
#  if( CAN_SAFE_BSW == STD_ON ) 
  if (canHwChannel >= kCanNumberOfChannels) /* PRQA S 2742 */ /* MD_Can_ConstValue */ /* CM_CAN_LL_100 */ /* COV_CAN_GENDATA_FAILURE */
  {
    Can_CallDetReportError(CAN_HW_ACCESS_ID, CAN_E_GENDATA); /* PRQA S 2880 */ /* MD_MSR_Unreachable */
  }
  else
#  endif
  {
...
/*  FETA Justification                                                  */
/****************************************************************************/
/* START_FETA_JUSTIFICATION

\ID FETA_CAN_HL01
\DESCRIPTION The for loop termination is determined depending on the configuration. Two scenarios can be applicable:
             Compile Time Configurations: The for loop termination is determined by a parameterized GenData access.
             Postbuild Configurations: The for loop termination is determined by a value returned by an GenData access via pointer in a postbuild loadable or postbuild selectable configuration.
...
\ID FETA_CAN_HL02
\DESCRIPTION The while loop has external termination condition computation (calls to CanLL_ModeTransition).
             The termination condition depends on the hardware to complete the requested action or until a timeout occurs.
             The loop execution time is limited by the TimeoutDetection feature and therefore definite.
\COUNTERMEASURE \D DSGN-Can22882
                \S SMI-1082795

\ID FETA_CAN_HL03
\DESCRIPTION The for loop termination is determined by the following calculation:
             activeSendObject = CanHL_GetActiveSendObject(initPara->mailboxHandle, 0u)
             activeSendObjectStop = activeSendObject + Can_GetMailboxSize(initPara->mailboxHandle)
             The for loop termination therefore depends on the MailboxSize which is determined by a parameterized GenData access.
\COUNTERMEASURE \N The GenData access is ensured via qualified use-case CSL01 of ComStackLib.
...
\ID FETA_CAN_HL05
\DESCRIPTION The for loop iterates over each element of a tx mailbox and checks for PDUs that are marked to be cancelled.
             The loop will terminate once a PDU was found (and either of the features multiplexed tx or hw fifo are active) or the end of the mailbox is reached.
             The maximum iteration count is determined by the mailbox size which is determined by parameterized GenData access.
\COUNTERMEASURE \N The GenData access is ensured via qualified use-case CSL01 of ComStackLib.

---------------- LL ------------------

\ID FETA_CAN_LL01
\DESCRIPTION The while loop termination is dependent on hardware access.
             The termination condition depends on the hardware to complete the requested action or until a timeout occurs.
             The loop execution time is limited by the TimeoutDetection feature and therefore finite.
\COUNTERMEASURE \D DSGN-Can22882

\ID FETA_CAN_LL02
\DESCRIPTION The for loop iterates over test patterns (members of a 32bit unsigned int array: CanMemCheckValues32bit).
             Those patterns will be used to write and read back values from hardware register to check for memory corruption.
\COUNTERMEASURE \N The for loop termination is either at constant maximum value (3) or earlier if a hardware malfunction is detected. 
                   Therefore the for loop terminates always after between 0 and 3 iterations.

\ID FETA_CAN_LL03
\DESCRIPTION The for loop iterates over the memory size of a mailbox or until a corrupt memory section is detected in hardware.
\COUNTERMEASURE \N The iterated variable j is used to access the corresponding memory section of the mailbox in 32bit steps.
                   The size of a mailbox is either statically 16 byte (8 byte data + 8 byte header -> termination condition: j < 4): 32bit = 4 byte and 4 * 4 byte = 16 byte = size of mailbox
                   or is determined by parameterized GenData access: Can_GetMaxDataLenOfMailbox (data length in byte) divided by 4 (hardware access is done in steps of 32 bit -> 4 byte) 
                                                                     + 8 byte header (also divided by 4)
                   The for loop termination is either at constant maximum value or earlier if a hardware malfunction is detected (ret_val != kCanFalse).

\ID FETA_CAN_LL04
\DESCRIPTION The for loop iterates over the bytes (wordwise - 32 bit -> 4 byte therefore curWord << 2u) of the transmit data of a frame.
\COUNTERMEASURE \N The termination of the for loop depends on the frameLen struct member of a transmission data struct.
                   The value of this parameter is determined from the PduInfo passed to the DrvCan by the CanIf and accessed via parameterized GenData access.
                   The validity of this parameter is checked during Can_Write.

\ID FETA_CAN_LL05
\DESCRIPTION The while loop termination condition is (curByte & 0x03u) != 0u while curByte is incremented in every iteration (starting from 0).
\COUNTERMEASURE \N The loop will always terminate after 4 iterations.

\ID FETA_CAN_LL06
\DESCRIPTION The for loop termination condition depends on a value read from a hardware register or until the maximum data length of a mailbox is reached,
             which is determined by parameterized GenData access. 
\COUNTERMEASURE \N fElmSize/rElmSize is determined by DlcToFrameLenght((rx<Basic/Full>Para->fElm.E1) & 0x000F0000UL) >> 16u.
                   The result of this can be 64 at maximum independent of the value read from hardware.
                   (indexL << 2u) < fElmSize/rElmSize) since the indexL value is shifted by 2 for each iteration
                   the loop will always terminate after a maximum amount of 16 iterations. (iteration 17 -> indexL = 16; indexL >> 2 = 64 -> 64 < 64 terminates the loop).

\ID FETA_CAN_LL07
\DESCRIPTION The while loop terminates when fillLevel reaches zero. fillLevel is decremented each iteration and it's initial value is read from hardware reigsters.
\COUNTERMEASURE \N Structurally the loop will always terminate independent of the value read from the register or hardware malfunction.
                   fillLevel is an 8 bit unsigned integer resulting in a maximum amount of 255 loops.

\ID FETA_CAN_LL08
\DESCRIPTION The for loop termination is determined by the following calculation:
             Can_GetStopAdrOfShmAdr(ch) - Can_GetStartAdrOfShmAdr(ch)) >> 2
             Both variables of the calculation are determined by a parameterized GenData access.
\COUNTERMEASURE \S SMI-246328

\ID FETA_CAN_LL09
\DESCRIPTION The while loop runs until there are no more new data flags pending or all 64 FullCAN buffers have been checked once.
\COUNTERMEASURE \N The loop will always terminate after a maximum amount of 64 iterations when idx will no longer be < 2.

\ID FETA_CAN_LL10
\DESCRIPTION The while loop runs until there are no more transmission occured flags pending or all Tx buffers have been checked once.
\COUNTERMEASURE \N The loop will always terminate after a maximum amount of 32 iterations when hwObjHdl will no longer be < kCanTXBC_NDTB (32).

END_FETA_JUSTIFICATION */ 
