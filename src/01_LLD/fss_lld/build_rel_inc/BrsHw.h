 ...
 *********************************************************************************************************************/
  ...
  #define BRS_FORCE_ACTIVE

  #define BRS_ISYNC()              _isync()
  #define BRS_DSYNC()              _dsync()

  #define BRS_MOVE_FROM_CSFR(x)    _mfcr(x)
  #define BRS_MOVE_TO_CSFR(x, y)   BRS_DSYNC(); _mtcr(x,y); BRS_ISYNC()

  #define BRS_SET_SP(x)            __asm( "mov.aa  %%a10, %0" ::"a" (x))

  #define BRS_FUNC_DECLARATION(rettype, attribute, functionName, arguments0, arguments1) \
    __inline__ rettype functionName (arguments0, arguments1)
...
#define BRS_ASSEMBLER_CODESECTION(sectionName)    __asm("  .section ."BRS_EXPAND(sectionName)", \"ax\"");

                                        
#define BRS_ASSEMBLER_ALIGN(x)                    __asm("  .balign "BRS_EXPAND(x)"");

#define BRS_ASSEMBLER_EXTERNDECLARATION(symbol)   /* not necessary */
...
#define BRS_ASSEMBLER_STARTUP_CODE() \
   __asm("  mfcr     %d8, lo:65052");                             /* get current Core ID *//* D8 = current core id */\
   __asm("  movh.a   %a14, hi:BrsMain_CoreConfig"); \
   __asm("  lea      %a14,[%a14]lo:BrsMain_CoreConfig");          /* get start address of BrsMain_CoreConfig[x] *//* A14 = &BrsMain_CoreConfig */\
   ...
   __asm("  movh.a   %a12,hi:Brs_SizeOfbrsMain_CoreType");        /* get the size of brsMain_CoreType */\
   __asm("  ld.a     %a12,[%a12]lo:Brs_SizeOfbrsMain_CoreType");  /* A12 = sizeof(brsMain_CoreType) */\
...
#define BRS_SAFETY_WATCHDOG_INDEX  -1
  ...
  #define BRS_SAFETY_WATCHDOG_ARRAY_INDEX 3x
...
/* Core Synch purpose*/
volatile uint8  CpuInitCompletedSem[3];
/**********************************************************************************************************************
