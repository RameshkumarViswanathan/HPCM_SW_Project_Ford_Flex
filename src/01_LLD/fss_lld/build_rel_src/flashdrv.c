...
#  define TRAP_PREFIX( class )

/* Compiler intrinsic abstraction */ /* PRQA S 3453 TAG_CompilerIntrinsicMacros */ /* MD_MSR_FctLikeMacro */
# define FLASH_ISYNC()              _isync()
# define FLASH_DSYNC()              _dsync()
# define FLASH_MFCR(regaddr)        _mfcr(regaddr)
# define FLASH_MTCR(regaddr, val)   _mtcr((regaddr), (val))
...
} tFlashHeader __attribute__ ((packed)); /* PRQA S 2071, 3448 */ /* MD_FblFlash_2071, MD_FblFlash_3448 */
...
/* PRQA S 0554 1 */ /* MD_FblFlash_0554_Traptable */
...
/* Trap Table ****************************************************************/
/* PRQA S 0341, 3412 TAG_FlashAsmTrapHandler */ /* MD_FblFlash_0341, MD_MSR_19.4 */
#  define DEFINE_TRAP(i, handler)            \
   __asm (".globl __trap_" #i);                \
   __asm ("__trap_" #i ":");                   \
   __asm ("svlcx");                            \
   __asm ("movh.a %a15,hi:" #handler "");      \
   __asm ("lea %a15,[%a15]lo:" #handler "");   \
   __asm ("mov %d4,%d15");                     \
   __asm ("calli %a15");                       \
   __asm ("rslcx");                            \
   __asm ("rfe");                              \
   __asm (".align 5")
/* PRQA L:TAG_FlashAsmTrapHandler */
/*
 ...
 */
   /* PRQA S 0286, 1006 TAG_Flash_AsmTrapTable */ /* MD_FblFlash_0286, MD_MSR_1.1 */
   //TechM
   __asm ("                                     \n\
         ...
         TriCore_trap_table:                    \n\
        ");
   /* PRQA L:TAG_Flash_AsmTrapTable */
   /* PRQA S 1006 9 */ /* MD_MSR_1.1 */
   DEFINE_TRAP(0u, FlashTrapMMU);        /* Trap class 0 (MMU) */
   DEFINE_TRAP(1u, FlashTrapProtection); /* Trap class 1 (Internal Protection Traps) */
   DEFINE_TRAP(2u, FlashTrapInstruction);/* Trap class 2 (Instruction Errors) */
   DEFINE_TRAP(3u, FlashTrapContext);    /* Trap class 3 (Context Management) */
   DEFINE_TRAP(4u, FlashTrapBus);        /* Trap class 4 (System Bus and Peripheral Errors) */
   DEFINE_TRAP(5u, FlashTrapAssertion ); /* Trap class 5 (Assertion Traps) */
   DEFINE_TRAP(6u, FlashTrapSystem);     /* Trap class 6 (System Call) */
   DEFINE_TRAP(7u, FlashTrapNMI);        /* Trap class 7 (Non-Maskable Interrupt) */
   __asm (".text");
...
static void FlashRedirectTraps( tFlashParam * flashParam ) /* PRQA S 3673 */ /* MD_FblFlash_3673_3678 */
{
   ...
   /* Set redirected trap base register */ /* PRQA S 0428, 0305 1 */ /* MD_FblFlash_0428_Traptable, MD_FblFlash_0305_Traptable */
   FLASH_MTCR(FLASH_BTV, (tFlashUint32)&TriCore_trap_table);
