    ...
    SchM_Enter_PduR_PDUR_EXCLUSIVE_AREA_0();
    {
      ...
      if(core_id < (uint8) PDUR_MAX_NUMBER_OF_CORES)
      {
        ...
        if(0u == PduR_GetCounterOfSpinlockRam(spinlockIdx, memIdx)[core_id])
        {
          ...
          while((Appl_TryToGetSpinlock(PduR_GetAddrLockVariableOfSpinlockRam(spinlockIdx, memIdx)) != E_OK) && (retryCounter < retryCounterValue))  /* COV_PDUR_SPINLOCK_TIMEOUT */ /* SBSW_PDUR_CSL03 */ /* FETA_PDUR_02 */
          {
            retryCounter++;
          }
