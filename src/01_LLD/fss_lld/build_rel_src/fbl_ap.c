 ...
 *               flash operations.
 **********************************************************************************************************************/
static V_MEMRAM1 vuint16 V_MEMRAM2 flashlight=0;
static V_MEMRAM1 vuint16 V_MEMRAM2 CAN_writeStatus=0;
void ApplFblTask( void )
{
   ...
   if (flashlight >= 10u)
   {
      ...
      dummy.swPduHandle = 0;
    CAN_writeStatus = Can_Write(0, &dummy);  // comment out for XCP testing
    dummy.id = 0x124; 
    CAN_writeStatus = Can_Write(2, &dummy);  // comment out for XCP testing
   //  dummy.id = 0x125; 
   //  CAN_writeStatus = Can_Write(4, &dummy);  // comment out for XCP testing    
