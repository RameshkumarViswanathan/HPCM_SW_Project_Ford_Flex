...
#if defined( FBL_ENABLE_OTA )
/** Size of the SWASH */
# define FBL_DIAG_SWASH_SIZE     32u

# if defined( FBL_ENABLE_VALSTRUCT )
/* Defines are copied from FbLib_ValStruct */ /* PRQA S 3453 TAG_FblDiag_3453_1 */ /* MD_MSR_FctLikeMacro */
/*** Validation structure defines ***/
#  define FBL_VALSTRUCT_PROLOG_SIZE                         (FBL_VALSTRUCT_TAG_SIZE + FBL_VALSTRUCT_VERSION_SIZE + FBL_VALSTRUCT_BLOCKCOUNT_SIZE)
...
#  define FBL_VALSTRUCT_BLOCKADDRESS_OFFSET(index)          (FBL_VALSTRUCT_BLOCKCOUNT_OFFSET + FBL_VALSTRUCT_BLOCKCOUNT_SIZE + ((index) * FBL_VALSTRUCT_BLOCKINFO_SIZE))
...
#if defined( FBL_ENABLE_OTA )
# define kDiagStateRc210Allowed           kDiagStateMask(kDiagStateIdxRc210Allowed)
# define GetRc210Allowed()                GetDiagState(kDiagStateIdxRc210Allowed)
# define SetRc210Allowed()                SetDiagState(kDiagStateIdxRc210Allowed)
# define ClrRc210Allowed()                ClrDiagState(kDiagStateIdxRc210Allowed)

# define kDiagStateUseInactivePartition   kDiagStateMask(kDiagStateIdxUseInactivePartition)
# define GetUseInactivePartition()        GetDiagState(kDiagStateIdxUseInactivePartition)
# define SetUseInactivePartition()        SetDiagState(kDiagStateIdxUseInactivePartition)
# define ClrUseInactivePartition()        ClrDiagState(kDiagStateIdxUseInactivePartition)

# define kDiagStateApplicationValidated   kDiagStateMask(kDiagStateIdxApplicationValidated)
# define GetApplicationValidated()        GetDiagState(kDiagStateIdxApplicationValidated)
# define SetApplicationValidated()        SetDiagState(kDiagStateIdxApplicationValidated)
# define ClrApplicationValidated()        ClrDiagState(kDiagStateIdxApplicationValidated)
#endif /* FBL_ENABLE_OTA */
...
# define FBLDIAG_START_SEC_CONST
# include "MemMap.h" /* PRQA S 5087 */  /* MD_MSR_MemMap */
/** Verification primitive configuration for security class DDD (/wo CRC total)*/
V_MEMROM0 static V_MEMROM1 SecM_VerifyOperationType V_MEMROM2 verifyConfigHashOnly[1] =
{
   {
...
/** Verification primitive configuration for security class DDD (/wo CRC total)*/
V_MEMROM0 static V_MEMROM1 SecM_VerifyOperationType V_MEMROM2 verifyConfigHashWithAddress[1] =
{
   {
      /* Use configured primitive */
      SecM_VerifyClassDDD,
...
#endif
#if defined( FBL_ENABLE_OTA ) && \
    defined( FBL_DIAG_ENABLE_DOWNLOAD_TO_INACTIVE_PARTITION )
static FBL_DIAG_MAIN_HANDLER(FblDiagMainHandlerRcUseInactiveMemPart);
static FBL_DIAG_MAIN_HANDLER(FblDiagMainHandlerRcActivateInactiveMemPart);
#endif /* FBL_ENABLE_OTA && FBL_DIAG_ENABLE_DOWNLOAD_TO_INACTIVE_PARTITION */
...
#endif
#if defined( FBL_ENABLE_OTA )
static void FblDiagGetVsAddresses(tFblAddress * vsAddresses, vuintx * vsIndex);
static tFblResult FblDiagCalculateHash(tFblAddress address,
                                       tFblLength length,
                                       vuint8* hash,
                                       FL_ReadMemoryFctType readMemFct,
                                       V_MEMROM1 SecM_VerifyOperationType V_MEMROM2 V_MEMROM3 * pOperation,
                                       SecM_AddrType transferAddr);
#endif /* FBL_ENABLE_OTA */
...
#if defined( FBL_ENABLE_OTA ) && \
    defined( FBL_DIAG_ENABLE_DOWNLOAD_TO_INACTIVE_PARTITION )
   ,kServiceMainHandlerRcUseInactiveMemPart
   ,kServiceMainHandlerRcActivateInactiveMemPart
#endif /* FBL_ENABLE_OTA && FBL_DIAG_ENABLE_DOWNLOAD_TO_INACTIVE_PARTITION */
#if defined( FBL_DIAG_ENABLE_UPDATE_SEC_BYTES )
...
#if defined( FBL_ENABLE_OTA ) && \
    defined( FBL_DIAG_ENABLE_DOWNLOAD_TO_INACTIVE_PARTITION )
   ,FblDiagMainHandlerRcUseInactiveMemPart
   ,FblDiagMainHandlerRcActivateInactiveMemPart
#endif /* FBL_ENABLE_OTA && FBL_DIAG_ENABLE_DOWNLOAD_TO_INACTIVE_PARTITION */
#if defined( FBL_DIAG_ENABLE_UPDATE_SEC_BYTES )
...
#if defined( FBL_ENABLE_OTA )
   /* RoutineControl 0x210 is allowed from intitialization until SBL has been downloaded. */
   SetRc210Allowed();
#endif /* FBL_ENABLE_OTA */
 ...
 *  FblDiagMainHandlerRcUseInactiveMemPart
 **********************************************************************************************************************/
/*! \brief       RoutineControl - Use Inactive Memory Partition subroutine
 *  \pre         SBL not yet activated
 *  \param[in]   pbDiagBuffer Pointer to the data in the pbDiagBuffer (without SID)
 *  \param[in]   diagReqDataLen Length of data (without SID)
 *  \return      Possible return values:
 *               - kFblOk: Service processed successfully (goto next state)
...
# endif
   ...
   {
      /* Activating the partition may take a while. For example if external flashes have to be re-initialized */
      DiagExRCRResponsePending(kForceSendResponsePending);

      if (vOtaM_ActivateInactivePartition(&part, VOTAM_ACTIVATION_STATE_USE_PARTITION) == E_OK)
      {
         /* Send positive response */
         pbDiagBuffer[kDiagFmtSubRoutineInfo] = (vuint8)((kDiagSubRoutineType1_Sync << 4u) | kDiagSubRoutineStatusCompleted);
         DiagProcessingDone(kDiagRslRcUseInactiveMemPart);
      }
      else
      {
         DiagNRCConditionsNotCorrect();
 ...
 *  FblDiagMainHandlerRcActivateInactiveMemPart
 **********************************************************************************************************************/
/*! \brief       RoutineControl - Activate Inactive Memory Partition subroutine
 *  \pre         Security Access unlocked, RC 0x210 has been executed successfully and application has been validated
 *  \param[in]   pbDiagBuffer Pointer to the data in the pbDiagBuffer (without SID)
 *  \param[in]   diagReqDataLen Length of data (without SID)
 *  \return      Possible return values:
 *               - kFblOk: Service processed successfully (goto next state)
...
# endif
   ...
   if ((!GetUseInactivePartition()) || (!GetApplicationValidated()))
   {
      result = kFblFailed;
      DiagNRCConditionsNotCorrect();
   }
   else
   {
      ...
      /* Security Access must be unlocked, application validity has already been checked with diag preconditions. */
      if (!GetSecurityAccess01())
      {
         result = kFblFailed;
         DiagNRCConditionsNotCorrect();
      }
      /* Get and activate inactive partition */
      else if (vOtaM_ActivateInactivePartition(&part, VOTAM_ACTIVATION_STATE_ACTIVATE_PARTITION) != E_OK)
      {
         result = kFblFailed;
         DiagNRCConditionsNotCorrect();
      }
      ...
      {
# if defined( FBL_DIAG_ENABLE_SAFE_SWAP )
         vOtaMSafeSwap_CheckpointReached(VOTAMSAFESWAP_DIAGNOSIS, VOTAMSAFESWAP_DIAGNOSIS_ACTIVATEPARTITION);
# endif /* FBL_DIAG_ENABLE_SAFE_SWAP */
         /* Swap active and inactive partitions */
         if (vOtaM_SwapPartition() == E_OK)
         {
            /* Routine successful, send response */
            pbDiagBuffer[kDiagFmtSubRoutineInfo] = (vuint8)((kDiagSubRoutineType1_Sync << 4u) | kDiagSubRoutineStatusCompleted);
            DiagProcessingDone(kDiagRslRcActivateInactiveMemPart);
         }
         else
         {
...
#if defined( FBL_ENABLE_OTA )
         SetApplicationValidated();
# if defined( FBL_DIAG_ENABLE_SAFE_SWAP )
         vOtaMSafeSwap_CheckpointReached(VOTAMSAFESWAP_DIAGNOSIS, VOTAMSAFESWAP_DIAGNOSIS_VERIFICATION);
# endif /* FBL_DIAG_ENABLE_SAFE_SWAP */
#endif /* FBL_ENABLE_OTA */
      }
...
static tFblResult FblDiagSecAccessKey(V_MEMRAM1 vuint8  V_MEMRAM2 V_MEMRAM3 * pbDiagBuffer, tCwDataLengthType diagReqDataLen, vuint8 secLevel)
{
   ...
   {
#if defined( FBL_ENABLE_OTA ) && \
    defined( FBL_DIAG_ENABLE_SAFE_SWAP )
      vOtaMSafeSwap_CheckpointReached(VOTAMSAFESWAP_DIAGNOSIS, VOTAMSAFESWAP_DIAGNOSIS_REGISTRATION);
#endif /* FBL_ENABLE_OTA && FBL_DIAG_ENABLE_SAFE_SWAP */
...
void FblDiagEcuReset(vuint8 resetOptions, vuint8 responseFlag)
{
   ...
   {      
      ...
      /*Update Ports to Tristate Before Jumping to BootLoader Or ECUReset*/
      Set_Ports_Tristate();      
      Timer_DelayTicks(20000);
      if(RESET_RESPONSE_SDS_REQUIRED == responseFlag) /*call FBL for Programming*/
      {
         CallFblStart();
      }
      else if(RESET_RESPONSE_ECURESET_REQUIRED == responseFlag) /*call SW Reset*/
      {
         ApplFblReset();
      }
      else
...
#if defined( FBL_ENABLE_OTA )
/***********************************************************************************************************************
 *  FblDiagGetVsAddresses
 **********************************************************************************************************************/
/*! \brief        Get all VS addresses of all logical block in a sorted order.
 *  \param[out]   vsAddresses    List of all VS addresses in a sorted order.
 *  \param[out]   vsIndex        Actual number of vsAddresses.
 **********************************************************************************************************************/
static void FblDiagGetVsAddresses(tFblAddress * vsAddresses, vuintx * vsIndex)
{
   vuintx localVsIndex = 0u;
   tFblAddress tmpAddr;
   vuintx i;
   vuintx j;
   tFblLbtBlockFilter blockFilter;
   ...
   /* Extract all VS addresses */
   FblLbtBlockFilterInit(&blockFilter);
   FblLbtBlockFilterSetBlockType(&blockFilter, kBlockTypeApplication);
   iterBlock = FblLbtBlockFirst(&blockFilter);
   while (FblLbtBlockDone() == FALSE)
   {
      vsAddresses[localVsIndex] = iterBlock.headerAddress;
      localVsIndex++;
   ...
   /* Sort addresses */
   for (i = 1u; i < localVsIndex; i++)
   {
      for (j = 0u; j < (localVsIndex - i); j++)
      {
         if (vsAddresses[j] > vsAddresses[j + 1u])
         {
            tmpAddr = vsAddresses[j];
            vsAddresses[j] = vsAddresses[j + 1u];
            vsAddresses[j + 1u] = tmpAddr;
         }
      }
   }

   *vsIndex = localVsIndex;
}

/**********************************************************************************************************************
 * FblDiagCalculateHash()
 *********************************************************************************************************************/
/*! \brief        Calculates hash over provided area.
 *  \param[in]    address        Start address of area for hash calculation
 *  \param[in]    length         Length of area for hash calculation
 *  \param[out]   hash           Buffer used to store the resulting hash
 *  \param[in]    readMemFct     Function used to access the memory
 *  \param[in]    pOperation     Verification operation
 *  \param[in]    transferAddr   Transfer address used for hash calculations including address and length
 *  \return       Possible return values:
 *                - kFblOk: Hash calculation was successful
 *                - kFblFailed: Hash calculation failed
 *********************************************************************************************************************/
                                       ...
                                       SecM_AddrType transferAddr)
{
   ...
   SecM_SignatureParamType localVerifyContext[1u];
   FL_SegmentInfoType localVerifySegmentInfo[1u];
   SecM_VerifyConfigListType localConfig;

   /* Configure segment which shall be verified */
   localVerifySegmentInfo[0].targetAddress      = (SecM_AddrType)address;
   localVerifySegmentInfo[0].transferredAddress = transferAddr;
   localVerifySegmentInfo[0].length             = length;

   localVerifyParam.segmentList.segmentInfo     = localVerifySegmentInfo;
   localVerifyParam.segmentList.nrOfSegments    = 1u;
   localVerifyParam.blockStartAddress           = localVerifySegmentInfo[0u].targetAddress;
   localVerifyParam.blockLength                 = length;
   localVerifyParam.verificationData            = hash;
   localVerifyParam.wdTriggerFct                = (FL_WDTriggerFctType)FblLookForWatchdog; /* PRQA S 0313 */ /* MD_FblDiag_0313_WDTriggerFctType */
   localVerifyParam.readMemory                  = readMemFct;

   /* Only perform hash calculation without CRC total */
   localConfig.pPrimitives.pOperation = pOperation;
   /* Provide context variable for processing */
   localConfig.pPrimitives.pContext   = localVerifyContext;
   localConfig.count                  = 1u;
   /* Force use of default internal workspace */
   localVerifyContext[0u].currentHash.sigResultBuffer   = (SecM_ResultBufferType)SEC_DEFAULT_WORKSPACE; /* PRQA S 0306 */ /* MD_FblDiag_0306 */
   localVerifyContext[0u].currentHash.length            = SEC_DEFAULT_WORKSPACE_SIZE;
# if defined( SEC_ENABLE_VERIFICATION_DATA_LENGTH )
   localVerifyContext[0u].currentDataLength             = SEC_DATA_LENGTH_NULL;
# endif /* SEC_ENABLE_VERIFICATION_DATA_LENGTH */

   if (SecM_VerificationWrapper(&localVerifyParam, &localConfig) == SECM_OK)
   {
      result = kFblOk;
   }
...
# if defined( FBL_DIAG_ENABLE_SAFE_SWAP )
/***********************************************************************************************************************
 *  FblDiagValidatePartition
 **********************************************************************************************************************/
/*! \brief       Validates and activates the partition
 *  \param[in]   partitionId  ID of the partition
 *  \return      Possible return values:
 *               - E_OK: Validating and activating was successful
 *               - E_NOT_OK: Validating and activating failed
 **********************************************************************************************************************/
Std_ReturnType FblDiagValidatePartition(vOtaM_PartitionIdType partitionId)
{
   Std_ReturnType result = E_NOT_OK;
   vuint8 calculatedSwash[FBL_DIAG_SWASH_SIZE];
   vuint8 applicationSwash[FBL_DIAG_SWASH_SIZE];
   vOtaM_PartitionIdType srcPartitionId;
   vOtaM_PartitionInfoType srcPartitionType;

   srcPartitionType = vOtaM_GetSourcePartition();
   srcPartitionId = srcPartitionType.Id;

   /* Work with inactive partition to get the application swash */
   MemDriver_SetAddressArea(vOtaM_GetAddrAreaIdOfPartition(srcPartitionId));

   if (ApplFblGetApplicationSwash(applicationSwash) == kFblOk)
   {
      /* Work with active partition to calculate swash of destination */
      MemDriver_SetAddressArea(vOtaM_GetAddrAreaIdOfPartition(partitionId));

      if (FblDiagCalculateSwash(calculatedSwash, FBL_DIAG_SWASH_CHECK_VALSTRUCT_AND_DATA) == E_OK)
      {
         result = vOtaMSafeSwap_ValidateActivePartition(calculatedSwash, applicationSwash);
      }
   }
   else
   {
      /* Ensure that FBL works with active partition */
      MemDriver_SetAddressArea(vOtaM_GetAddrAreaIdOfPartition(partitionId));
   }
...
/***********************************************************************************************************************
 *  FblDiagSwapPartition
 **********************************************************************************************************************/
/*! \brief        Swaps the partition
 *  \param[in]    partitionId    ID of the partition
 *  \return       Possible return values:
 *                - E_OK: Swapping was successful
 *                - E_NOT_OK: Swapping failed
 **********************************************************************************************************************/
Std_ReturnType FblDiagSwapPartition(vOtaM_PartitionIdType partitionId)
{
   Std_ReturnType result = E_NOT_OK;
   vuint8 calculatedSwash[FBL_DIAG_SWASH_SIZE];
   vuint8 applicationSwash[FBL_DIAG_SWASH_SIZE];
   ...
   if (ApplFblGetApplicationSwash(applicationSwash) == kFblOk)
   {
      if (FblDiagCalculateSwash(calculatedSwash, FBL_DIAG_SWASH_CHECK_VALSTRUCT_AND_DATA) == E_OK)
      {
         result = vOtaMSafeSwap_Swap(MemDriver_GetAddressArea(), calculatedSwash, applicationSwash);
      }
   }

   /* Work with active partition after A/B swap */
   MemDriver_SetAddressArea(vOtaM_GetAddrAreaIdOfPartition(partitionId));
...
# if defined( FBL_DIAG_SWASH_PRE_SWAP_CHECK_TYPE )
/**********************************************************************************************************************
 * FblDiagSwashPreSwapCheck()
 *********************************************************************************************************************/
/*! \brief        Check the preconditions of swap action.
 *  \param[in]    srcPartitionId    ID of the source partition.
 *  \param[in]    destPartitionId   ID of the destination partition.
 *  \return       Possible return values:
 *                - E_OK: Precheck successfully executed - SWAP shall be executed
 *                - E_NOT_OK: Precheck failed - SWAP shall not be performed
 *********************************************************************************************************************/
Std_ReturnType FblDiagSwashPreSwapCheck(vOtaM_PartitionIdType srcPartitionId, vOtaM_PartitionIdType destPartitionId)
{
   Std_ReturnType result = E_NOT_OK;
   vuint8 calculatedSwash[FBL_DIAG_SWASH_SIZE];
   vuint8 applicationSwash[FBL_DIAG_SWASH_SIZE];
   ...
   if (ApplFblGetApplicationSwash(applicationSwash) == kFblOk)
   {
      if (FblDiagCalculateSwash(calculatedSwash, FBL_DIAG_SWASH_PRE_SWAP_CHECK_TYPE ) == E_OK)
      {
         /* Change result to OK, will be changed in case swashes do not match */
         result = E_OK;

         /* Compare swashes */
         for (index = 0u; index < FBL_DIAG_SWASH_SIZE; index++)
         {
            if (calculatedSwash[index] != applicationSwash[index])
            {
               result = E_NOT_OK;
               break;
            }
   ...
   /* Use dest partition ID to work with active partition */
   MemDriver_SetAddressArea(vOtaM_GetAddrAreaIdOfPartition(destPartitionId));

   return result;
}
# endif /* FBL_DIAG_SWASH_PRE_SWAP_CHECK_TYPE */

/**********************************************************************************************************************
 * FblDiagCalculateSwash()
 *********************************************************************************************************************/
/*! \brief        Calculates the SWASH
 *  \details      -
 *  \param[in]    calculatedSwash   Buffer used to store the resulting SWASH value
 *  \param[in]    swashCheck        Specify the type of SWASH check, which shall be performed
 *  \return       Possible return values:
 *                - E_OK: Swapping was successful
 *                - E_NOT_OK: Swapping failed
 *********************************************************************************************************************/
Std_ReturnType FblDiagCalculateSwash(vuint8 * calculatedSwash, tFblDiagSwashCheck swashCheck)
{
   Std_ReturnType result = E_NOT_OK;
   tFblResult fblResult = kFblOk;
   vuint8 vsBuffer[FBL_VALSTRUCT_MAX_SIZE];
   vuint8 rhBuffer[FBL_DIAG_SWASH_SIZE * (FBL_LBT_BLOCK_COUNT - 1u)];
   tFblAddress tmpAddr;
   tFblLength tmpLength;
   tFblAddress vsAddresses[FBL_LBT_BLOCK_COUNT - 1u];
   ...
   /* Get all VS addresses in a sorted order */
   FblDiagGetVsAddresses(vsAddresses, &vsIndex);

   for (i = 0u; i < vsIndex; i++)
   {
      /* Copy prolog into vsBuffer */
      (void)FblReadProm(vsAddresses[i], vsBuffer, FBL_VALSTRUCT_PROLOG_SIZE);

      /* Copying remaining part of val struct based on numbers of entries in prolog */
      vsSegCount  = FblValStructGetBlockCount(vsBuffer); /* Get number of entries */
      vsTotalSize = FblValStructGetTotalSize((tFblLength)vsSegCount); /* Calculate VS size */
      (void)FblReadProm((vsAddresses[i] + FBL_VALSTRUCT_PROLOG_SIZE), &(vsBuffer[FBL_VALSTRUCT_PROLOG_SIZE]), (vsTotalSize - FBL_VALSTRUCT_PROLOG_SIZE));

      if (swashCheck == FBL_DIAG_SWASH_CHECK_VALSTRUCT_AND_DATA)
      {
         /* Calculate Hash for each segment - overwrite written data in RAM */
         for (segIndex = 0u; segIndex < vsSegCount; segIndex++)
         {
            tmpAddr   = FblValStructGetBlockAddress(vsBuffer, segIndex);
            tmpLength = FblValStructGetBlockLength(vsBuffer, segIndex);

            fblResult |= FblDiagCalculateHash(tmpAddr,
                                 tmpLength,
                                 &(vsBuffer[FBL_VALSTRUCT_BLOCKCHECKSUM_OFFSET(segIndex)]),
                                 (FL_ReadMemoryFctType)FblReadProm,
                                 verifyConfigHashOnly,
                                 tmpAddr);
         }
      }

      /* Calculate root hash */
      fblResult |= FblDiagCalculateHash((tFblAddress)&vsBuffer, /* PRQA S 0306 */ /* MD_FblDiag_0306 */
                           vsTotalSize,
                           &(rhBuffer[(FBL_DIAG_SWASH_SIZE * i)]),
                           (FL_ReadMemoryFctType)FblReadRam,
                           verifyConfigHashWithAddress,
                           vsAddresses[i]);
   }

   /* Compute swash */
   fblResult |= FblDiagCalculateHash((tFblAddress)&rhBuffer, /* PRQA S 0306 */ /* MD_FblDiag_0306 */
                        (FBL_DIAG_SWASH_SIZE * vsIndex),
                        calculatedSwash,
                        (FL_ReadMemoryFctType)FblReadRam,
                        verifyConfigHashOnly,
                        (tFblAddress)&rhBuffer);  /* PRQA S 0306 */ /* MD_FblDiag_0306 */
